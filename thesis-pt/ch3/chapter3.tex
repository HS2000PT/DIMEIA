...
tickers, construído pelo processo idêntico); reconstruir a base de conhecimento de recuperação a partir do
corpus multi-ano fica para trabalho futuro.}
\end{table}
...
\begin{tikzpicture}[
  ...
  \node[raw] (s1) at (0,0) {%
    ...
    $+$ preços de fecho diários\\ (yfinance)};
  ...
  \node[rep] (s3) at (7.1,0) {%
    ...
    contexto: vol20 .021,\\ mom5 $+$.122, ret $+$.017\\[3pt]
...
Dois detalhes do passo de embedding merecem ser explícitos, porque cada um responde a uma pergunta que um
examinador faria com razão. O primeiro é o que $\mathrm{embed}(h)$ de facto calcula. O Sentence-BERT é um
...
\toprule
\tabhead{Coluna} & \tabhead{O que é} & \tabhead{Valor de exemplo} \\
\midrule
\texttt{vol20} & volatilidade pré-evento: dp dos 20 log-retornos diários que terminam em $d{-}1$ & $0.021$ \\
\texttt{mom5} & momentum de cinco dias até $d{-}1$ (log-retorno cumulativo) & $+0.122$ \\
\texttt{ret\_event} & log-retorno do dia do evento, conhecido no fecho de $d$ & $+0.017$ \\
\texttt{headline\_len} & número de caracteres na manchete & $55$ \\
\texttt{sector} & indicador de setor one-hot & tech \\
\texttt{embedding} & vetor Sentence-BERT de $384$-d da manchete & $[-0.005, 0.004, \dots]$ \\
\midrule
\texttt{label} \emph{(alvo)} & $|$retorno anormal vs SPY sobre $(d,d{+}3]| \ge 2\%$ & $1$ (material) \\
\bottomrule
\end{tabular}
\end{table}
...
como substituto de aconselhamento profissional, uma limitação enunciada com clareza em vez de escondida.
