...
% Company marks live in slides/logos/companies/, produced by scripts/fetch_slide_logos.py from
% the same files the deployed app draws. Same graceful degradation as \techlogo: no file, no box.
...
\begin{frame}{What it watches --- and why two of the twelve are not tech}
\centering\vspace{0.5mm}
{\footnotesize\textbf{\textcolor{blue!60}{XLK --- technology}}}\par\smallskip
...
{\small Until August 2026 the watchlist was \textbf{ten}, and \textbf{nine of them sat in XLK}.
So the question ``was it the sector?'' nearly always came back with the same answer --- not
because that was the answer, but because the watchlist gave the sector factor nothing to vary
against. \textbf{XOM (energy) and JNJ (healthcare) were added to fix that}, and the effect showed
up immediately in live output: \textbf{XOM $-0.98\%$ on a day its sector was $+0.93\%$} --- the
sector pulling the \emph{other} way, which nine technology names could not have shown.
\par\smallskip
{\footnotesize This is a change to the \emph{evaluation}, not to the layout.}}
\end{columns}
\end{frame}
...
\end{columns}
\vspace{1mm}\par\centering\footnotesize On multi-year FNSPID under the deployed causal constraint:
\textbf{P@5 $0.513$ vs $0.259$ floor} (margin $+0.254$). The symmetric scale test gives $0.595$;
direction consistency $0.71$ sits at the $0.69$ floor (theme, not direction).
\end{frame}
...
\midrule
Random ranking (floor) & 0.378 & 0.379 \\
...
\small Within a 5-alerts/day budget, triage lifts precision \textbf{by $1.67\times$} ($0.632$ vs $0.379$) \emph{on held-out data from the training corpus} --- but \textbf{no text model beat volatility}. A follow-up ablation added \emph{five} more cheap context features (market regime, momentum, standardised reaction, downside risk): PR-AUC $0.535$, again \textbf{no gain}. A \emph{fair} re-test (tuned $C$, PCA-reduced text, a FinBERT domain encoder) still never beat volatility, so the negative is \textbf{robust}, not an under-fit artefact. The signal is market context; reported as it fell.
\end{block}
\end{frame}

\begin{frame}{The bug that showed the model was never deciding}
\small
\begin{columns}[T]
\column{0.5\textwidth}
\textbf{What happened.} Nvidia rose sharply on a supply announcement. The channel carried
\textbf{two lesser stories} about Nvidia that day and \textbf{never carried that one}.
\medskip

\textbf{Why.} The daily cap of two alerts per company was served \textbf{in arrival order}. Two
immaterial morning stories exhausted the budget; the afternoon story was dropped \textbf{without
a trace in any log}.
\column{0.5\textwidth}
\begin{block}{\small The uncomfortable part}
\small The instrument was already there. Triage exists to rank by materiality, and lifts
precision from $0.379$ to $\mathbf{0.632}$ inside a 5-alert budget --- and \textbf{the cap never
consulted it}. The score was measured, then ignored.
\end{block}
\smallskip
{\footnotesize \textbf{The first fix did not work, and that is the interesting part.} Sorting the
cap by score can only reorder candidates present \emph{together}, and the scan emits one headline
per company per cycle --- so two stories competing for the same quota never coexist. Across a day
the order stayed arrival order. The total is governed by the \textbf{global 5/day budget}. With it
enabled, each company's first alert has no floor and the second must clear $0.64$; $0.49$ remains
only as provenance of the sweep that derived both values.}
\end{columns}
\vfill
{\footnotesize\textbf{And the fix had its own bug.} Comparing \emph{rendered alerts} to catch
repeated stories collides on shared template, so two \emph{different} stories would silently
suppress one another. Caught by an existing test. It now compares \textbf{headlines}, and fails
\textbf{open} when a headline is too short to judge.}
\end{frame}
...
\column{0.45\textwidth}
\small
A genuine screenshot, not a mockup. \textbf{Every card opens with a sentence, then
the numbers behind it.}
\begin{itemize}\small
  \item \textbf{Rarity is a count, not a probability} --- ``only $5$ of the last $249$ days
        moved this much''. Turning $z$ into a probability needs normality; returns have fat
        tails, so it would fail on exactly the days that matter.
  \item \textbf{Quiet is evidenced, not asserted} --- a calm day shows the count that makes
        it calm.
  \item \textbf{When the two measures disagree, both are shown} --- Microsoft: below the
        alert threshold against its own $20$ days, yet only $5$ of $249$ moved as much.
\end{itemize}
{\footnotesize One click away: chart with events, the full market/sector/company split,
precedents, and the alerts \emph{exactly} as Telegram received them. The promise appears
\textbf{once}. Latency is \textbf{measured}, and absent when the timestamps are not there.}
\end{columns}
\end{frame}
...
% The width cap is not decoration: square icons and wordmarks share these rows, and with height
% alone a wordmark renders ~7x wider than an icon, dominates the line and pushes the last badge
% onto a row of its own. Capping both dimensions with keepaspectratio bounds every logo to the
% same box, so a row reads as one row.
...
\begin{frame}{Result 9: the gate was measured in production, and it did not transfer}
\begin{columns}[T]
\column{0.52\textwidth}
\small
Every triage decision is logged and, days later, labelled with what the price actually did ---
same rule as training. Over \textbf{530} matured decisions:
\medskip

\begin{tabular}{lc}
\toprule
\textbf{Alerts the gate\dots} & \textbf{were material} \\
\midrule
\textbf{kept} ($p \ge 0.5$) & $0.592$ \\
\textbf{suppressed} ($p < 0.5$) & $\mathbf{0.647}$ \\
\bottomrule
\end{tabular}
\medskip

{\footnotesize The gate is on the \emph{wrong} side, though not significantly
($z=-1.28$, $p=0.20$).}

\column{0.48\textwidth}
\begin{block}{\small Which failure is it?}
\small Two failures look identical here and need opposite fixes. If the score still
\textbf{ranks}, recalibrate (two parameters). If it does not rank, recalibration is useless ---
a sigmoid is \textbf{monotone}, it preserves the order exactly.
\end{block}
\smallskip
{\footnotesize Measured: \textbf{ROC-AUC $0.494$}, cluster CI $[0.391, 0.601]$ over $145$
(ticker, day) units. Centred on chance. \textbf{No order to preserve.}}
\end{columns}
\vfill
{\footnotesize\textbf{Why, and it is not a broken model --- it is a redundant one.} Materiality
among logged decisions runs at $0.626$ against $0.378$ in training, because decisions are only
logged for headlines that \emph{already} cleared the relevance and freshness filters. The cheap
upstream stages had removed what the learned stage was trained to remove. \textbf{A model
evaluated in isolation and deployed behind filters was never evaluated on the distribution it
would see.} The gate stays on as a \emph{volume} control (it suppresses $\sim$3/5) and because it
is the instrumented part --- switching it off would end the measurement that found this.}
\end{frame}

\begin{frame}{Where the AI is: four layers, and the fourth is visible}
\begin{columns}[T]
\column{0.50\textwidth}
\small
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Layer} & \textbf{Nature} \\
\midrule
1 Data & measured \\
2 Statistics & deterministic \\
3 \textbf{Learning} & \textbf{trained models} \\
4 \textbf{Generation} & \textbf{grounded LLM} \\
\bottomrule
\end{tabular}
\medskip

{\footnotesize Layer 3 is a sentence transformer plus a calibrated classifier trained here on
$79{,}753$ examples. Layer 4 \emph{combines} their outputs --- the one task where the state space
outgrows any template library.}

\column{0.50\textwidth}
\begin{block}{\small The division that makes it defensible}
\small The engines write the \textbf{facts}; the model writes the \textbf{language}. It never
observes the market. It receives an evidence bundle whose every record declares its provenance
(\emph{measured} / \emph{computed} / \emph{model}) --- and \textbf{no record is ever
\emph{generated}}, asserted by test.
\end{block}
\end{columns}
\vfill
{\footnotesize\textbf{The claim, made checkable rather than promised:} each cited identifier in
the prose is a control that opens the record behind it. Sentence $\to$ fact $\to$ engine $\to$
file. \emph{``The model does not know what happened. It is told --- and every number it writes is
checked against the evidence that produced it.''}}
\end{frame}

\begin{frame}{The guard: what it blocks, what it admits, and what it does not cover}
\begin{columns}[T]
\column{0.46\textwidth}
\small
\begin{tabular}{lr}
\toprule
Adversarial attacks blocked & $23/23$ \\
\textbf{Faithful controls passed} & $\mathbf{8/8}$ \\
Generated sections conforming$^{*}$ & $27/27$ \\
Delivered with a violation & $\mathbf{0}$ \\
\bottomrule
\end{tabular}
\medskip

{\footnotesize The \textbf{second} row is the one that matters: without it, a guard that rejected
\emph{everything} would score $23/23$ and look flawless.

\smallskip
$^{*}$\,Sampled from one run of six reports: the \emph{rate} carries over, the counts do not. The
first two rows are deterministic and reproduce exactly; the zero must hold in \emph{every} run.}

\column{0.54\textwidth}
\begin{block}{\small Two guarantees, not one --- and the weaker one is declared}
\small The \textbf{pushed} alert uses a closed-vocabulary \emph{allowlist}. The \textbf{pulled}
report uses a blocklist plus per-sentence numeric binding. Different delivery risk, different
guarantee. Numbers are closed-set in \emph{both}.
\end{block}
{\footnotesize\textbf{Stated as a limitation, not discovered by an examiner:} the linguistic
defence is a blocklist and blocklists lose in the limit; citation \emph{relevance} is unchecked;
and the adversarial exercise was \textbf{interrupted} --- two of six perspectives ran and no
verification stage completed, so the measured strength is a \textbf{lower bound}.}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item The generated layer's \textbf{language} guarantee is weaker than the alert path's, by design and by declaration; its adversarial review was interrupted, so its measured strength is a lower bound.
  \item The recent retrieval corpus is a \textbf{short} window; on multi-year FNSPID the causal task gives P@5 $0.513$ against a $0.259$ floor. The $0.595$ figure is from the symmetric scale protocol, which allows later candidates.
...
\begin{itemize}
  ...
  \item \textbf{RQ2 (yes):} under the deployed causal constraint, P@5 $0.513$ against a $0.259$ floor (margin $+0.254$); $0.595$ is the symmetric scale test. Impact measured without lookahead.
  \item \textbf{RQ3 (faithful yes; useful open):} explanations cannot diverge from the computation; human usefulness is future work.
  \item \textbf{RQ4 (no on text; mechanism yes \emph{offline}):} triage $\approx 4\times$ within-budget precision on held-out data, but volatility stayed unbeaten --- the \emph{second} fair ``learned vs simple'' test the transparent choice won (first: Isolation Forest vs \emph{z}-score, $F_1$ $0.271$ vs $0.530$). Measured in production the same gate ranks at chance (Result 9), so the mechanism claim is an evaluation-set claim and is stated as one.
\end{itemize}
\vfill
...
\begin{itemize}
  ...
  \item \textbf{Your trained model lost to the volatility baseline. Failure?} No: the comparison was \emph{pre-committed} and answers RQ4 with evidence. Triage gives $\approx 4\times$ within-budget precision on held-out data, and the deployed variant uses exactly the features that carry the signal.
  \item \textbf{So does the ML work in production?} \emph{As a selector, no, and I measured it} (ROC-AUC $0.494$). As a volume control, yes. The reason is instructive: the upstream filters already removed most of what it was trained to remove, so it is redundant rather than broken --- a lesson about placing a learned component \emph{inside} a pipeline.
  \item \textbf{Does the system learn automatically?} No. It does \emph{inference} with fixed weights and \emph{collects labels}; retraining is possible and documented but has never run. Calling that continual learning would be an overclaim.
