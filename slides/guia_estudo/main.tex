...
\vspace{1mm}
\begin{itemize}
  \item \key{Dashboard público} — \texttt{investigator.streamlit.app}: os dois gatilhos e a
        avaliação, no browser. O retrieval é \key{semântico}: o MESMO MiniLM da tese em
        \key{ONNX} ($\sim$23 MB, sem torch; paridade com o SBERT validada — cosseno médio 0,992)
        sobre uma fatia curada de 2.016 manchetes FNSPID 2018--2023. A página "Markets now"
        mostra os \key{mesmos alertas} que o canal recebe (mesma config, detetor e texto).
  \item \key{Canal Telegram + timer} — o GitHub Actions varre a watchlist \key{de 30 em 30 min}
        em horário de mercado US (best-effort) e publica alertas explicados no canal.
        Anti-repetição no dia + anti-repetição de notícias ($\leq 2$ dias) e
        anti-duplicado em feriados (só avalia com sessão nova).
  \item \key{Bot interativo} — \texttt{/watch TSLA}, \texttt{/list}, \texttt{/stop}: cada
        utilizador gere a sua watchlist (SQLite local; \emph{long-polling}, sem servidor).
        Off por defeito no runner; fail-open (nunca parte a produção).
  \item \key{À prova de wifi} — se a demo ao vivo falhar: \texttt{python scripts/demo.py} é
        offline e determinística (reproduz o $+6{,}46\%$ do Cap.~3).
\end{itemize}
\vspace{1mm}
