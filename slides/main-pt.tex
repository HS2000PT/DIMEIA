...
\begin{block}{Dois desafios distintos}
\small
\begin{enumerate}
  \item Distinguir um movimento \emph{genuinamente} invulgar da volatilidade comum.
  \item Ler o que uma notícia pode \emph{significar}, a partir de como eventos semelhantes se comportaram antes.
\end{enumerate}
\end{block}
\end{columns}
\end{frame}
...
\small Dentro de um orçamento de 5 alertas/dia, a triagem quase \textbf{quadruplica a precisão} ($0.632$ vs $0.163$) --- mas \textbf{nenhum modelo de texto bateu a volatilidade}. Uma ablação de seguimento juntou \emph{cinco} features de contexto baratas (regime de mercado, momentum, reação padronizada, risco de queda): PR-AUC $0.535$, de novo \textbf{sem ganho}. O sinal é o contexto de mercado; reportado tal como se revelou.
\end{block}
\end{frame}
...
\small
\begin{itemize}
  \item Uma página, uma aba por ticker da watchlist --- uma captura genuína, não um mockup.
  \item Risco de fundo do modelo de triagem treinado, todos os dias, sem precisar de notícia.
  \item Gráfico anotado com os eventos detetados + uma tabela de histórico --- o \textbf{mesmo} registo que o canal Telegram recebeu.
  \item Apenas camada gratuita: GitHub Actions (cron) + Streamlit Community Cloud.
\end{itemize}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item O corpus de recuperação é uma janela \textbf{recente} da camada gratuita; o estudo de \emph{triagem} usou de facto o FNSPID multi-ano (14/15 tickers --- a Meta é ``FB'' no corpus).
...
\begin{itemize}
  ...
  \item \textbf{RQ2 (sim, recuperação):} a recuperação semântica encontra precedentes análogos muito acima das linhas de base; impacto medido sem lookahead.
