...
The root holds **eight loose files and nothing else** (plus the five dotfiles that live at
a repository root by convention: `.gitignore`, `.gitattributes`, `.python-version`,
`.env.example`, and the gitignored `.env`). Each of the eight is there because a tool reads
it at the repository root, not because it was left behind:

| File | Why it cannot move |
|------|--------------------|
| `Procfile` | Heroku reads it at the repository root |
| `requirements.txt` | it is what makes the Python buildpack detect the app; also `pip install -r requirements.txt` in `ci.yml` and `alerts.yml` |
| `app.json` | Heroku app manifest, read at the root |
| `pyproject.toml` | packaging (`pip install -e .`) plus the `pytest` and `ruff` configuration |
| `README.md`, `CITATION.cff` | GitHub conventions: entry point and citation metadata |
| `CLAUDE.md`, `AGENTS.md` | agent context, read at the root by the tools used on this project |

Everything else lives in a named folder:

```
tese-pt/       canonical Portuguese dissertation (6 chapters + 2 appendices)
  slides/        defence slides (Beamer)
  guia/          the study guide (single study source)
  figures/       figures, generated by scripts/figuras/ and by the evaluation scripts
tese-eng/      the English mirror — kept in total sync with tese-pt/ (see AGENTS.md)
paper/         IEEE paper (IEEEtran) distilled from the dissertation

investigator/  system code, one package per component (investigator/triage/ = the trained
               ML component, QI3; investigator/qi4/ = the contrastive encoder, QI4)
api/           FastAPI service: data routes over investigator/, plus the report and analyst
web/           the served single-page client (Lightweight Charts v5, vendored, Apache 2.0)
app/           earlier Streamlit generations, kept because thesis figures cite them;
               verdict.py and method.py are still used — the API calls them, so the
               sentences the reader sees cannot drift from the tested Python behind them
models/        trained models (joblib, versioned)
scripts/       demo.py + run_alerts.py + the evaluation, build and gate scripts
tests/         automated tests

config/        alerts.yaml (watchlist, thresholds, quality knobs)
  requisitos/    requirements-app / requirements-ml / requirements.lock
data/samples/  small committed samples (large data gitignored, rebuilt by scripts)

docs/          all documentation (see docs/README.md for the index)
  contexto/      session continuity: ESTADO_ATUAL, TASKS, BRIEF_REESCRITA, PROMPT_TAREFA
  design/        how_to_run, going_live, deployment, architecture, data card
  evaluation/    auto-generated evaluation artefacts (never edit by hand)
  decisions/     citation log, page audit, learning notes, glossary
  planos/        plans and audits
  defence/       defence preparation
  study/         the usefulness-study protocol

archive/       what is no longer read or executed — see archive/README.md for the
               folder-by-folder reason. Nothing was deleted: it all moved with `git mv`.
```
...
- Python **3.12** in a virtual environment: `bash scripts/setup_env.sh` installs the **light** stack
  ...
  `sentence-transformers`, in `config/requisitos/requirements-ml.txt`) is needed only for the real SBERT paths and installs with
