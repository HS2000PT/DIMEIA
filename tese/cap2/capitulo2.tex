...
\emph{Quem é esta pessoa, e o que é que a literatura já sabe sobre a forma como ela decide?}

A escala foi dada no Capítulo~\ref{cap:introducao}: a maior parte das famílias americanas detém
...
particulares de uma plataforma sem comissões reforçaram posições em vez de fugirem
\autocite{welch2022robinhood}, o que contraria a caricatura do pequeno investidor que só reage em
...
A segunda é mais desconfortável, e é a que fundamenta este trabalho inteiro.
\textcite{barber2008glitters} mostram que os investidores particulares são compradores líquidos de
...
avalia todas, e recai sobre aquilo que lhe saltou à vista. A atenção é de tal modo central neste
...
Os três sinais que capturam a atenção do investidor, ou seja a notícia, o volume invulgar e o
movimento extremo do dia, são exatamente os sinais que este sistema deteta. A diferença é o que
acontece a seguir. Hoje, esses sinais chegam à pessoa em bruto, e o que a literatura documenta é que
ela compra a seguir. O que este trabalho tenta é interpor, entre o sinal e a decisão, o contexto que
permite julgá-lo: se aquilo é invulgar para aquela empresa, se foi a empresa ou o mercado, e o que
se seguiu em casos parecidos. A Figura~\ref{fig:ctx_atencao} resume esta cadeia.
...
\begin{tikzpicture}[
  ...
  \node[s, below=0.35cm of n] (v) {\textbf{Volume}\\\footnotesize invulgar};
...
\end{tikzpicture}
\caption[Da atenção à decisão]{Os três sinais que a literatura identifica como captadores de atenção
\autocite{barber2008glitters} são os mesmos que este sistema deteta. A seta a tracejado é o caminho
que existe hoje, em que o sinal chega em bruto. A caixa central é o que este trabalho acrescenta.}
\label{fig:ctx_atencao}
\end{figure}

Uma última nota de enquadramento. A adoção de inteligência artificial no setor financeiro deixou de
ser experimental e passou a ser generalizada, tanto do lado das instituições como do lado dos
produtos de retalho \autocite{ccaf2026aifs}. Isso significa que a pergunta relevante já não é se
estas técnicas vão chegar ao investidor particular, porque estão a chegar, mas sim em que condições
de transparência é que chegam.
...
Resolveram, em parte, e de maneiras diferentes. Convém olhar para o que existe com um critério fixo,
...
Convém olhar para três linhas em particular, porque cada uma falha por uma razão diferente e essas
...
As \textbf{apps de sentimento} pontuam notícias como positivas ou negativas e mostram o resultado.
...
dinheiro por uma carteira. É uma pergunta legítima e não é esta. A literatura reconhece-lhes
benefícios genuínos, nomeadamente a melhoria da diversificação e a contenção de enviesamentos
comportamentais \autocite{dacunto2019robo, cardillo2024robo}, mas o modo de funcionamento é o
oposto do deste trabalho: eles \emph{agem por} o investidor, com lógica proprietária, ao passo que
aqui se procura explicar e deixar a decisão à pessoa. Acresce que aconselhar sobre investimentos tem
obrigações regulatórias que este trabalho evita por desenho, como se detalha na
...
e não se consegue conferir, o que é precisamente a falha de interpretabilidade contra a qual a
literatura da área avisa \autocite{lipton2018mythos, rudin2019stop}. A literatura sobre confiança em
sistemas automáticos acrescenta que o problema não é as pessoas confiarem de menos: é confiarem
\emph{na medida errada}, o que exige que o sistema mostre em que se apoia \autocite{lee2004trust}.
...
\emph{Como é que se decide, de forma defensável, que um dia foi diferente dos outros?}
...
Há uma restrição prática que condiciona toda esta escolha e que convém enunciar cedo: em finanças a
deteção é dominada por métodos \emph{não supervisionados}, porque anomalias rotuladas são escassas
\autocite{ahmed2016financial}. Não existe um conjunto de dados em que alguém tenha marcado, dia a
...
estável, e essa suposição merece ser confrontada em vez de escondida. A literatura de volatilidade
mostra que os mercados exibem \emph{agrupamento de volatilidade}, ou seja a tendência de os períodos
calmos e os agitados chegarem em séries \autocite{engle1982arch, bollerslev1986garch}. Uma janela
...
problema, mas trata-o, e o Capítulo~\ref{cap:avaliacao} mede a alternativa em que a janela pondera
observações recentes com mais peso.
...
\emph{Como é que um computador decide que duas notícias falam do mesmo?}
...
documento pela frequência dos termos que contém, e a linha de trabalho que dele decorre, até ao BM25
\autocite{robertson2009bm25}, continua a ser uma linha de base forte em recuperação de informação.
É transparente e é rápido, mas é cego a sinónimos: para este tipo de representação, ``subiu'' e
``valorizou'' são símbolos sem relação nenhuma. Em texto financeiro há ainda uma variante
...
\addlinespace
Modelos de domínio \newline \autocite{araci2019finbert, yang2020finbert}
...
Existem modelos treinados especificamente para texto financeiro \autocite{araci2019finbert,
yang2020finbert}. A suposição natural é que sejam melhores nesta tarefa por serem do domínio certo.
Essa suposição não é automaticamente verdadeira, e o Capítulo~\ref{cap:avaliacao} mede-a em vez de a
assumir.

Convém situar este trabalho dentro da literatura mais vasta de processamento de linguagem natural
aplicado a finanças. As revisões dessa área até meados da década de 2010 \autocite{kearney2014textual}
cobrem as duas primeiras gerações; a terceira chegou depois, e é de onde este trabalho tira a sua
representação. Existe ainda uma linha de investigação substancial que usa texto para \emph{prever}
...
\emph{Encontrada uma representação, como se recupera o que interessa, e como se avalia isso?}
...
\emph{Encontrados casos análogos, como se mede honestamente o que aconteceu ao preço a seguir?}

O instrumento é o \emph{estudo de evento}, introduzido por \textcite{fama1969adjustment} e
normalizado para retornos diários por \textcite{brown1985daily}, que fixaram a prática de medir
retornos sobre janelas curtas após o acontecimento. É essa prática que fundamenta as janelas de mais
um, mais três e mais cinco dias usadas neste trabalho.
...
literatura de finanças resolveu isto há décadas encolhendo a estimativa na direção de um valor de
referência. A forma mais simples usa um peso fixo \autocite{blume1971risk}, o que tem o inconveniente
de encolher com a mesma força uma estimativa boa e uma má. A alternativa que este trabalho adota
pondera o encolhimento pela \emph{imprecisão} da própria estimativa \autocite{vasicek1973beta}, de
modo que uma estimativa fiável é praticamente preservada e uma estimativa frágil é puxada para o
valor de referência. O detalhe está na Secção~\ref{sec:met_decomposicao}.

Por fim, o enquadramento teórico que justifica a recusa de prever. Se movimentos futuros fossem
sistematicamente previsíveis a partir de informação pública, essa informação já estaria refletida no
preço \autocite{fama1970efficient}. Esta é uma das razões pelas quais o sistema mede o que já
...
\emph{Mostrar casos anteriores em vez de uma regra: isso tem nome?}
...
reutilizando o que se sabe sobre eles, em vez de aplicar uma regra geral derivada de antemão. É
exatamente o que a terceira técnica deste trabalho faz.
...
\emph{Se o sistema decide, como é que mostra em que se baseou?}
...
Os dois métodos \emph{a posteriori} mais usados operam de formas distintas. O LIME ajusta um modelo
simples em torno de uma previsão individual, aproximando localmente o comportamento do modelo opaco
\autocite{ribeiro2016lime}. O SHAP atribui uma previsão às suas variáveis usando valores de Shapley
...
SHAP \autocite{lundberg2017shap}
  ...
  & Fundamentação teórica sólida; custo computacional e leitura exigente para um leigo \\
\addlinespace
Modelo transparente \newline \autocite{rudin2019stop}
  & Interpretável por construção
  & A explicação é o próprio cálculo, logo não pode divergir da decisão; limita a família de modelos disponível \\
\bottomrule
\end{tabular}
\end{table}
...
\emph{confiança excessiva}: \textcite{bansal2021whole} mostram que as pessoas aceitam explicações
apresentadas com confiança mesmo quando a resposta subjacente está errada. Daqui decorre uma
...
Acresce que afirmações de interpretabilidade exigem avaliação rigorosa e não bastam ser declaradas
...
\emph{Treinar um modelo é uma coisa. Mantê-lo a funcionar é outra. O que diz a literatura?}
...
por reforço do gradiente \autocite{friedman2001gbm} constituem o aprendiz não linear mais forte
disponível para dados tabulares desta natureza, e servem por isso de referência superior contra a
qual o modelo interpretável é medido no Capítulo~\ref{cap:avaliacao}.

A segunda é a da honestidade do número produzido. Para quem lê, uma pontuação só é honesta se for uma
\emph{probabilidade}. As pontuações em bruto de um classificador estão tipicamente mal calibradas, e a
calibração \emph{a posteriori}, ajustando por exemplo uma sigmoide sobre um bloco de validação
reservado, corrige esse problema \autocite{niculescu2005calibration}. A escolha da forma paramétrica
em detrimento da alternativa não paramétrica é deliberada: uma sigmoide de dois parâmetros não
consegue sobreajustar um bloco de validação modesto, ao passo que a regressão isotónica precisa de
bastante mais dados de calibração antes de a sua flexibilidade adicional compensar
\autocite{niculescu2005calibration}.

Uma terceira via, considerada e não adotada, merece registo porque a decisão foi tomada com
conhecimento dela. A predição conformal permite envolver um preditor de modo que devolva um
\emph{conjunto} de rótulos com uma garantia livre de distribuição e válida em amostras finitas
\autocite{vovk2005algorithmic}, e a variante que calibra sobre um bloco reservado, com a respetiva
correção de quantil para amostras finitas, está estabelecida \autocite{angelopoulos2023conformal}.
Importa ser preciso quanto ao alcance da garantia, porque é o mal-entendido mais comum a respeito
deste método: a cobertura é \emph{marginal}, ou seja vale em média sobre os casos, e não caso a caso.

A terceira questão é a de continuar a funcionar com o tempo. Um modelo implantado enfrenta alteração
da distribuição dos dados que vê, e o tratamento habitual disso em aprendizagem automática é a
literatura de \emph{deriva de conceito}, cuja revisão de referência estabelece a taxonomia da
mudança, sendo esta súbita, gradual, incremental ou recorrente, bem como as estratégias de deteção e
os métodos de adaptação \autocite{gama2014survey}. Este trabalho mede a deriva sobre os seus próprios
dados no Capítulo~\ref{cap:avaliacao}, em vez de a assumir ausente.

Há por fim um custo que não aparece em nenhuma métrica de qualidade e que a experiência com sistemas
em produção documenta bem. \textcite{sculley2015debt} argumentam que os componentes de aprendizagem
automática acumulam um custo de manutenção distintivo e em grande medida oculto, nomeadamente o
emaranhamento, em que mudar qualquer coisa muda tudo, as dependências de dados instáveis e não
declaradas, e a dívida de configuração. O Capítulo~\ref{cap:conclusoes} regressa a este ponto com um
exemplo concreto e medido deste próprio sistema, em que uma parte do ciclo de aprendizagem parou sem
que nenhum erro fosse registado.

\section{Síntese: a lacuna que este trabalho ocupa}
\label{sec:ctx_lacuna}

\emph{Postas as peças em cima da mesa, o que é que falta?}
...
\begin{description}
  ...
  \item[Não prever preços.] Nem direção, nem alvo, nem recomendação. Além da razão já dada, que é o
        que torna tudo verificável, há a razão teórica da Secção~\ref{sec:ctx_evento}: se movimentos
        futuros fossem previsíveis a partir de informação pública, essa informação já estaria
        refletida no preço \autocite{fama1970efficient}.
