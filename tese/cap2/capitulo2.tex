...
em média sobre os casos, e não caso a caso. Registe-se que \textcite{vovk2005algorithmic} não tratam isto
como limite intransponível, e dedicam uma secção a mostrar que a \emph{validade condicional} se pode
obter modificando a definição de preditor conformal. Convém dizer o que essa expressão ali
significa, porque não é o que o nome sugere: é validade \emph{dentro de categorias} fixadas à
partida, e não caso a caso, portanto estreita a média sem a eliminar. O que se afirma aqui é que a
variante corrente, e a que seria adotada sem trabalho adicional, dá a garantia marginal. A segunda precisão é a condição sob a qual
