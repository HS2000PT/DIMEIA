...
fixo na tradição de \textcite{blume1971risk}, foi testada e rejeitada por uma razão concreta: encolhe
com a mesma força uma estimativa boa e uma má, e num caso construído em que uma ação se move
exatamente ao dobro do mercado, um peso fixo de dois terços devolve $1.67$ onde a verdade é $2.0$.

\subsection{Um caso real, com todos os passos}

A Tabela~\ref{tab:met_decomposicao} mostra a decomposição de um dia real da AMD, regenerável pelo
comando do Apêndice~\ref{ap:reprodutibilidade}.
...
No mesmo dia, e sobre as 17 empresas vigiadas, a resposta variou: em 8 casos o motor foi a própria
...
estimação. Sobre as 17 empresas, o $R^2$ mediano foi $0.460$, e \textbf{numa delas foi negativo}, o
...
fórmula reduz-se à soma de cima. Comparar duas manchetes passa a ser \textbf{multiplicar e somar},
...
A Tabela~\ref{tab:met_cosseno} segue a conta para dois pares de manchetes retirados da base de
casos. São reais, e os valores são os que o sistema calcula.

\begin{table}[ht]
\caption[O cosseno entre duas manchetes reais]{Dois pares do lado oposto da escala. Repare-se na
...
\midrule
Primeira manchete & ``Shares of several companies in the technology, software and semiconductor
space are trading lower\ldots'' & ``Peloton Shares Tick To Session Low As Hearing Report Apple
Working On `Guided Workout'\ldots'' \\
\addlinespace
Segunda manchete & ``Shares of several technology, software and semiconductor companies are
...
que a soma final é ligeiramente negativa: as manchetes não são opostas em sentido nenhum, apenas não
...
todas as notícias do mesmo dia. Sobra uma única entrada que distingue duas manchetes da mesma
...
por manchete e é essa que responde à pergunta. Mas fixa desde já o que a variante sem texto pode e
...
\midrule
1 & \textbf{Entrada} & As nove entradas da linha: volatilidade $0.0127$, momento $0.0249$,
movimento do dia $0.0364$, comprimento do título $53$, e o setor em nove casas das quais só uma
está a $1$. \\
\addlinespace
...
$-0.253$ da volatilidade, $+0.236$ do setor, $+0.006$ do movimento do dia, mais os restantes e o
termo constante. Total: $\mathbf{+0.029}$. \\
\addlinespace
4 & \textbf{Sigmoide} & O total é convertido em algo entre $0$ e $1$:
$1/(1+e^{-0.029}) = \mathbf{0.507}$. \\
\addlinespace
...
A parcela de maior peso, $-0.253$, veio da volatilidade, e puxou a probabilidade \emph{para baixo}.
...
\begin{lstlisting}[label=lst:lookahead, caption={O teste que impede o futuro de entrar nas \emph{features}. As duas
...
inútil. Exigir apenas que as \emph{features} não mudem seria satisfeito por um sistema que
