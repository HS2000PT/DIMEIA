...
existia antes de ser construída.
...
sistema devolve $z = 0$ em vez de falhar. Uma ação parada é descrita como não tendo nada de
...
ou seja tudo o que mercado e setor não explicam, e é por isso que a soma fecha sempre.
...
A Figura~\ref{fig:met_cosseno} mostra as duas situações.
...
respeito ao tamanho do texto e não ao seu tema. Os dois valores são os do par real da
...
A incoerência é real e fica declarada. O que a atenua é que o efeito é \emph{simétrico entre os
modelos comparados}: todas as seis famílias da Secção~\ref{sec:av_qi3} são treinadas e avaliadas
contra o mesmo rótulo, portanto um rótulo mais ruidoso torna a tarefa mais difícil para todas por
igual e não favorece nenhuma. O que ele afeta é o \emph{nível} das métricas, não a ordenação, que
é o que a pergunta compara. Refazer o rótulo com betas encolhidos obrigaria a retreinar as seis e
a re-correr tudo o que delas depende, e o benefício seria um número diferente para a mesma
conclusão.
...
\emph{Que riscos é que um sistema destes cria, e o que é que este faz em relação a eles?}
...
é uma fronteira de \textbf{desenho}, traçada por prudência e não a partir de uma análise do direito
aplicável. Não houve parecer jurídico, a bibliografia deste trabalho não tem fontes legais, e
quem quisesse operar isto como serviço teria de o obter antes. Este descreve o que já aconteceu e mostra a
evidência ao lado, e a Secção~\ref{sec:ctx_escolhas} explica porque é que essa fronteira é de
desenho e não de modéstia.

Há aqui uma distinção que é preciso fazer em voz alta, porque a Figura~\ref{fig:sis_alerta} a
torna impossível de ignorar: o alerta que serve de exemplo, e que é um alerta real copiado do
registo, cita um título que diz \emph{``Prediction: Amazon Will Join Apple in the \$4 Trillion Club
Before 2030''}. Isso é uma previsão, com um alvo e com uma data, entregue no telemóvel de uma
pessoa por este sistema.

A garantia que este trabalho dá aplica-se ao que o sistema \textbf{escreve}, e não ao que ele
\textbf{cita}. O título é conteúdo de terceiros, reproduzido \emph{verbatim} porque é o facto que
desencadeou o alerta, e reproduzi-lo é o que torna o resto verificável: sem ele o leitor não pode
ir à fonte confirmar do que se trata. Tudo o que é do sistema, nesse mesmo alerta, respeita a
restrição, e vê-se nas três partes seguintes, que falam de casos passados observados, de
\emph{``not a prediction for this news''} e de probabilidade \textbf{em qualquer direção}.

Fica no entanto uma limitação que decorre disto e que não tem solução dentro das restrições
assumidas: \textbf{o filtro de relevância decide se o título é sobre a empresa, e não se ele é
factual}. Um título especulativo passa como qualquer outro. Distinguir notícia de opinião exigiria
um classificador que este trabalho não tem, e é uma das linhas que o
Capítulo~\ref{cap:conclusoes} deixa em aberto.

\paragraph{Fadiga de alertas, e o direito a não ser interrompido.} Um sistema que avisa de tudo
não é atento, é ruidoso, e ensina quem o recebe a ignorá-lo. Isto trata-se como um requisito e não
como um detalhe: existe um orçamento de cinco alertas por dia entre todas as empresas, um piso que
sobe a cada alerta repetido da mesma empresa, e supressão de histórias repetidas. Ao contrário dos
pisos, que saem de um varrimento de custos, \textbf{o cinco não foi derivado de medição nenhuma}:
é uma escolha, feita para que uma pessoa consiga ler tudo o que recebe, e a
Secção~\ref{sec:av_qi3} avalia a política \emph{dado} esse orçamento em vez de o justificar. O
Capítulo~\ref{cap:sistema} mostra o funil completo, e nove em cada dez varreduras não enviam nada.
O silêncio é uma decisão do sistema, e é por isso que a página web o torna inspecionável.

Isso resolve o problema para quem abre a página, e \textbf{não para quem só recebe as mensagens}.
No canal, um sistema calmo e um sistema avariado são indistinguíveis: cinco dias sem nada podem
ser um mercado sossegado ou um processo que morreu, e quem espera não tem como saber qual. O
sistema emite uma nota de abertura e um resumo de fecho em cada dia útil, precisamente para que
a ausência deles seja um sinal, mas isso é uma prova de vida indireta e não uma garantia. Fica
como limitação assumida do canal enquanto interface.

\paragraph{Confiança a mais, e confiança a menos.} Uma explicação não torna automaticamente uma
decisão melhor. A literatura mostra que a confiança falha nos dois sentidos, com o utilizador a
ignorar um aviso correto ou a aceitar um errado \autocite{lee2004trust}, e que a simples presença
de uma explicação aumenta a aceitação de respostas erradas \autocite{bansal2021whole}. É por isso
que este sistema mostra os casos passados um a um, com as datas e o desfecho de cada um, em vez de
os resumir a uma média que se lê como uma previsão. \textbf{Que isso consiga produzir confiança
apropriada é uma hipótese, e não um resultado}: sem o estudo com pessoas descrito na
Secção~\ref{sec:con_limitacoes}, não é possível distinguir um alerta ignorado de um alerta
indevidamente acreditado, porque ambos deixam o mesmo registo, que é nenhum.

\paragraph{E uma restrição legal que se descobriu a meio.} Três dos ficheiros que o repositório
distribui derivam de um conjunto de dados publicado sob uma licença que exige partilha nos mesmos
termos, e o modelo de documento usado nesta dissertação tem uma licença que exige o mesmo e ainda
proíbe uso comercial. Isso condiciona a licença do próprio código, que deixa de ser uma escolha
livre. Fica registado porque é o tipo de restrição que se descobre tarde e obriga a decidir com
pressa.
