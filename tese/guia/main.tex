...
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{app_v6_empresa.png}
\end{column}
\begin{column}{0.37\textwidth}
\small
Captura real, de 20 de agosto, com a sessão a decorrer.
\vspace{2mm}
\begin{itemize}
  \item A \key{frase} vem antes do número: é a regra de apresentação.
  \item A \key{repartição} responde à segunda pergunta. Aqui a ação desce,
        mas o mercado e a empresa estavam em alta: quem a puxa é o \textbf{setor}.
  \item Os \key{pontos} na curva são dias que o detetor assinalou; as \key{setas},
        os que deram alerta. A distância entre uns e outros é o custo das portas.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
A mesma página mostra, por baixo, \key{onde cada empresa parou} nesse dia.
\vspace{2mm}
\begin{center}
\includegraphics[width=0.95\textwidth,height=0.55\textheight,keepaspectratio]{app_v6_silencio.png}
\end{center}
\vspace{1mm}
{\footnotesize Se te perguntarem \emph{``e quando não envia nada?''}, é para aqui que
apontas: nove em cada dez varreduras não enviam, e cada etapa diz a porta e a margem que
faltou (a AMD parou em $0.42$ contra um piso de $0.45$).}
\end{frame}
...
\begin{center}
\textbf{o texto acrescenta ao melhor preditor que já existe?}
\end{center}
\vspace{2mm}
Medido \textbf{por cima} da tabela de consulta por empresa, que é esse melhor preditor:
\begin{center}
\key{$+0{,}012$} de PR-AUC, IC $[+0{,}004,\ +0{,}020]$ \quad ---\quad \textbf{exclui zero}
\end{center}
É a única vez neste trabalho em que uma representação de texto mostra valor mensurável.
\vspace{1mm}
...
\key{+0.012} IC $[+0.004, +0.020]$ & o texto \textbf{por cima} da melhor base: acrescenta & QI3 \\
