...
\title[Guia de estudo]{Guia de estudo --- InvestiGator}
...
Quando corre em produção faz \key{inferência} --- aplica o que aprendeu --- e \textbf{não} volta a
aprender.
\end{block}
\vspace{3mm}
\footnotesize
...
\key{re-treino} (voltar a treinar com dados novos --- não é feito).
\end{frame}
...
\begin{enumerate}
  ...
  \item \key{Já aconteceu, e o que se seguiu?} --- recuperação + medição
\end{enumerate}
\vspace{4mm}
...
Uma previsão não se confere --- nem no momento, nem depois.\\[2mm]
...
\key{0,662} & o prior de 13 constantes --- \textbf{bate} o modelo & QI3 \\
...
``Não sei por promessa --- sei por teste. Há um teste que altera o futuro e exige duas coisas ao
