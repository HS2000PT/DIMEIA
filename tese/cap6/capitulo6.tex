...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \node[sim] (q2) at (0,-2.0)   {\textbf{QI2} \\ Encontrar notícias passadas genuinamente
                                  parecidas?};
  \node[ver] at (3.55,-2.0)     {SIM};
  \node[num] at (7.3,-2.0)      {precisão@5 de $0.514$ contra $0.346$, $0.240$ e $0.126$;
                                  aguenta à escala, com $0.595$};
...
\gls{ARCH} e \gls{GARCH} formalizam \autocite{engle1982arch, bollerslev1986garch} e que a literatura
financeira posterior documentou nos retornos de ações, ao passo que um detetor treinado sobre o conjunto
...
\emph{o texto acrescenta ao melhor preditor que existe?} Medida assim, por cima da tabela de
consulta por empresa que a Secção~\ref{sec:av_ablacao} identifica como esse melhor preditor, a
resposta é \textbf{sim}, com $+0.012$ de PR-AUC e um intervalo $[+0.004, +0.020]$ que exclui zero.
É pequeno, é real, e é a única vez neste trabalho em que uma representação de texto mostra valor
mensurável.
...
\begin{enumerate}
  ...
  \item \emph{Avaliar cada componente contra linhas de base transparentes.} Cumprido para os três
        componentes em que uma linha de base existe, e num deles mais do que o prometido: o detetor
        foi comparado não só com uma regra fixa mas também com dois métodos aprendidos. Para a
        decomposição não há linha de base possível, e o que se fez em vez disso está dito na
...
\begin{tikzpicture}[font=\footnotesize, >={Stealth[length=2mm]},
  ...
  \foreach \xn/\xlim/\xfut/\xcus in {%
    ...
    2/{O alerta afirma mais evidência do que tem}/{Medir o impacto por notícia, e não por dia}/{preços ao minuto},%
