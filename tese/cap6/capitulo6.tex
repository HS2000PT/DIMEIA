...
E existe uma avaliação de cada uma das quatro técnicas, contra alternativas mais simples, com os
...
perceber porquê. Os dois métodos aprendidos definem normalidade pela geometria dos dados, o que é
uma vantagem quando há muitas variáveis a interagir \autocite{chandola2009anomaly}; aqui há uma
série de retornos por empresa, e a estrutura que interessa é temporal, não geométrica. A janela
deslizante capta diretamente o agrupamento de volatilidade que caracteriza estas séries
\autocite{engle1982arch, bollerslev1986garch}, ao passo que um detetor treinado sobre o conjunto
...
palavras \autocite{robertson2009bm25}, do que escolher ao acaso e do que devolver as mais recentes
($0.514$ contra $0.346$, $0.240$ e $0.126$), e o resultado aguenta à escala do corpus completo
($0.595$). Com uma ressalva que ficou clara ao medir: a semelhança capta o \textbf{tema}, não a
\textbf{direção}.

Essa ressalva merece ser lida à luz da teoria e não como um defeito da implementação. Se a direção
de um movimento futuro fosse recuperável a partir do tema de uma notícia pública, essa informação
já estaria refletida no preço \autocite{fama1970efficient}. O que se mediu é, portanto,
aproximadamente o que se devia esperar, e é essa a razão pela qual o produto apresenta os
precedentes como observações do passado e nunca como indicação do que vem a seguir.
...
contradissesse. Não a contradiz, e convém ser preciso sobre porquê. Esses trabalhos usam
tipicamente o corpo dos artigos ou fluxos de texto muito maiores, extraem estruturas de
acontecimento em vez de comparar frases, e avaliam noutras condições. O que aqui se mostra é mais
estreito e também mais útil para quem constrói: \textbf{sob esta restrição de dados}, ou seja
manchetes de fontes gratuitas, o sinal textual não acrescenta ao que a volatilidade recente já
dizia. É um resultado sobre um regime de recursos, e não sobre a linguagem.
...
avaliação e não bastam ser declaradas \autocite{doshivelez2017rigorous}, e o motivo não é
formalismo: o efeito de uma explicação sobre uma pessoa não é dedutível da sua construção. Sabe-se
que o risco não é as pessoas confiarem de menos mas confiarem na medida errada
\autocite{lee2004trust}, e que uma explicação apresentada com confiança pode levar alguém a aceitar
uma resposta errada \autocite{bansal2021whole}. Este sistema é construído para reduzir esse risco,
mostrando a evidência em vez de emitir um veredicto, mas \emph{que o consiga} é uma hipótese por
verificar e não um resultado.
...
manchetes da mesma empresa no mesmo dia partilham o mesmo valor por construção. Medido sobre os
...
O alerta passou a contar dias e não manchetes, o que torna a afirmação verdadeira. Mas a causa de
...
exigiria dados de preço ao minuto, que as fontes gratuitas não dão.
...
\autocite{kearney2014textual}, e uma manchete é, por construção, a parte do documento onde mais
...
Esta dependência de aproximações não é uma particularidade deste trabalho: em finanças a deteção é
dominada por métodos não supervisionados precisamente porque anomalias rotuladas são escassas
\autocite{ahmed2016financial}. Quem quiser um alvo melhor tem de o construir com julgamento humano,
...
dependência não declarada descrita atrás \autocite{sculley2015debt}, e foi encontrada a olhar, não
por um alarme.
...
O que foi implantado como porta de decisão foi a variante \emph{sem} texto, escolhida por ter a
melhor PR-AUC entre as que cabiam no contentor. Só que essa variante recebe informação de nível de
\emph{empresa}, e a única entrada que distingue duas manchetes da mesma empresa no mesmo dia é o
comprimento do título. A sua pontuação é quase constante dentro de cada empresa \textbf{por
aritmética}, e não por acidente: em $84\%$ das decisões o resultado estava determinado antes de a
notícia ser lida (Secção~\ref{sec:av_inevitavel}).
...
\begin{itemize}
  \item o \textbf{resultado da \gls{QI}3 mantém-se}. A variante com texto tinha mesmo $384$ números
        por manchete e perdeu à volatilidade. A pergunta de investigação foi respondida;
...
uma classe de custo que não aparece em nenhuma métrica de qualidade: dependências de dados não
declaradas e emaranhamento entre componentes, em que a validade de uma peça depende de suposições
...
Nada nesta lista é uma ideia solta. Cada item fecha uma limitação da secção anterior, e a
Figura~\ref{fig:con_futuro} mostra essa correspondência, porque uma lista de trabalho futuro que não
derive das limitações medidas é um pedido de desculpas com outra forma.
...
\begin{tikzpicture}[
  ...
  \foreach \xn/\xlim/\xfut/\xcus in {%
    ...
    1/{O modelo implantado é uma tabela de consulta: $84\%$ das decisões fechadas antes de ler a notícia}/{Entradas que variem com a manchete}/{baixo: já existem no sistema},%
...
se resolvem com pessoas, e são por isso as primeiras da lista.}
...
\begin{enumerate}
  ...
  \item \textbf{Dar ao modelo de decisão entradas que variem com a notícia.} É a correção direta da
        ...
        volatilidade e do setor, mudam de manchete para manchete.
  ...
  \item \textbf{Dar ao utilizador um conjunto em vez de um número.} A predição conformal permite
        ...
        ferramenta que recusa afirmar mais do que sabe. Foi considerada e não adotada, e a razão
        fica registada para quem retome: a garantia é \emph{marginal}, ou seja vale em média sobre
        os casos e não caso a caso, o que é precisamente a leitura errada que um utilizador faria.
        Usá-la exigiria explicar esta distinção no produto, e isso é desenho de interação por
        fazer, não uma questão de implementação.
\end{enumerate}
...
ao modelo que está implantado. Comecei este trabalho a assumir que o caminho para um bom resultado
era acrescentar sofisticação. Acabo-o com três medições a dizer o contrário, e com a convicção de
...
verifica. Neste trabalho ela não existiu em nenhuma das três comparações. A diferença é que aqui
não foi uma posição assumida à cabeça: foi a conclusão a que a medição chegou, três vezes, contra
o que eu esperava.
