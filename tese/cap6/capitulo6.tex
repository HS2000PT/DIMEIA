...
perceber porquê. Os dois métodos aprendidos definem normalidade pela geometria dos dados
\autocite{liu2008isolation, breunig2000lof}, e foram desenhados para situações com muitas variáveis
...
sobreposição de palavras, do que escolher ao acaso e do que devolver as mais recentes ($0.514$ contra
$0.346$, $0.240$ e $0.126$), e o resultado aguenta à escala do corpus completo ($0.595$). A linha de
base lexical medida é a mais simples da sua família, e não uma função de ordenação afinada, o que
delimita o alcance da afirmação. Com uma ressalva que ficou clara ao medir: a semelhança capta o \textbf{tema}, não a
\textbf{direção}.
...
extraem estruturas de acontecimento, do tipo actor, ação e objeto, e treinam uma rede para prever a
...
\begin{enumerate}
  ...
  \item \textbf{Dar ao utilizador um conjunto em vez de um número.} A predição conformal permite
        ...
        temporal, com as manchetes a chegarem por ordem. É a mesma hipótese que este trabalho
...
verifica. Duas das minhas três comparações são instâncias disso: o \emph{z}-score contra dois
detetores cuja decisão não se lê, e a regressão logística contra as árvores com \emph{boosting}.

A terceira não é, e convém não a arrumar na mesma prateleira só porque a conclusão rima. A tabela de
