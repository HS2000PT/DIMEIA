...
% Segue o subtítulo da tese, que passou a nomear as duas técnicas. Aqui fica a versão curta, sem
% o público, porque o slide de capa já o tem no corpo e a linha não pode partir em duas.
\subtitle{\normalsize Deteção de anomalias e recuperação de precedentes em alertas verificáveis}
...
\includegraphics[height=0.68\textheight]{app_v6_empresa.png}
\end{column}
\begin{column}{0.35\textwidth}
\small
\textbf{1. É invulgar?}\\
{\footnotesize Em palavras, antes do número: $169$ dos últimos $250$ dias
moveram-se tanto ou mais. E \emph{``so far today''}: a sessão ainda decorria.}
\vspace{3mm}

\textbf{2. Foi a empresa?}\\
{\footnotesize \textbf{Não.} A ação desce $0.41\%$, mas o mercado e a própria
empresa estavam em alta. O que a puxa para baixo é o \textbf{setor}. A
percentagem sozinha mandava procurar uma razão onde não havia nenhuma.}
\vspace{3mm}
...
\end{center}
\vspace{1mm}
\begin{center}\includegraphics[width=0.97\textwidth,height=0.52\textheight,keepaspectratio]{app_v6_silencio.png}\end{center}

{\footnotesize As decisões de um dia, por ordem. \textbf{É a parte que uma
demonstração ao vivo não mostra}: nove em cada dez varreduras não enviam nada.}
