...
\subtitle{\normalsize Alertas de mercado com evidência verificável para o investidor de retalho}
...
\textbf{É a única das quatro sem resultado medido:} não há resposta certa contra a qual comparar.
\end{frame}
...
pela \textbf{empresa} antes de a manchete ser lida.
\end{center}
\vspace{1mm}
...
\begin{enumerate}
  ...
  \item \textbf{A técnica mais simples ganhou três vezes.}\\
        \footnotesize Ao Isolation Forest e ao modelo que lê o texto, que são casos de
        \emph{simples contra complexo}. E uma tabela de constantes ganhou ao modelo implantado,
        que é um caso diferente: ali o modelo até era interpretável, e perdeu por \textbf{falta de
        informação}, não por excesso de complexidade.
\end{enumerate}
\vspace{3mm}
\begin{center}
...
\begin{itemize}
  ...
  \item \textbf{O atraso é da fonte, não da infraestrutura}: $\approx$2.5\,h até detetar,
        1\,s até entregar.
  \item \textbf{Nada volta a ser treinado.} A base de casos cresce sozinha, mas os pesos do
        modelo são os que foram aprendidos uma vez. A deriva é medida e não é corrigida.
\end{itemize}
\end{frame}

\begin{frame}{O que faria a seguir, e porquê essa ordem}
Nenhum destes itens é uma ideia solta: \textbf{cada um fecha uma limitação que foi medida}.
\vspace{3mm}
\begin{center}
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]},
  ...
  \foreach \xn/\xlim/\xfut/\xcus in {%
    ...
    1/{O modelo implantado é uma tabela de consulta}/{Entradas que variem com a manchete}/{baixo},%
...
\vspace{2mm}
\begin{center}
\small As duas que custam \textbf{calendário} e não computação são as primeiras da lista.
\end{center}
\end{frame}
