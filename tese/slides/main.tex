...
Os \emph{betas} são estimados \textbf{só com dias anteriores}, e encolhidos conforme a
\textbf{precisão} da estimativa: uma estimativa frágil é puxada para a referência.\\
\textbf{Ressalva:} as três parcelas somam sempre, porque a da empresa é o resto. Isso não
prova que estejam bem estimadas. O $R^2$ mediano é $0.460$, e num dos casos é negativo.
\end{frame}
...
\begin{tikzpicture}[font=\tiny, x=7.0cm, y=0.245cm]
  ...
  { \node[left, black!70] at (0.235,\i) {\t};
    \draw[line width=2.2pt, color=\c] (\a,\i) -- (\b,\i); }
