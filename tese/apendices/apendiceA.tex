...
em que o tema e a direção discordam, que é onde a Secção~\ref{sec:av_qi2} mostra que o sistema é
mais fácil de ler mal.

\textbf{O contrabalanço cruza dois fatores, e não um.} O primeiro é a ordem. O segundo é qual das
metades serve de condição A, e existe por uma razão que só apareceu ao prepará-lo: as duas
condições têm de usar alertas \emph{diferentes}, senão a segunda mede memória; mas se a mesma
metade fosse sempre a A, qualquer diferença entre A e B ficaria confundida com uma das metades ser
mais fácil. Cruzados os dois, cada conjunto aparece o mesmo número de vezes de cada lado e o efeito
do estímulo cancela-se entre participantes em vez de se somar ao da condição.

\textbf{A pergunta que interessa.} A garantia de que cada frase do alerta se abre no facto que a
sustenta está verificada por máquina e por construção, e \textbf{nunca por uma pessoa}. Mas a
afirmação do produto é uma travessia que um leitor faz: dada uma frase com âncora, consegue abrir o
facto e julgar se ele a sustenta, sozinho? Se ninguém a conseguir fazer, a contribuição é
verdadeira e inútil. É essa a pergunta que o estudo existe para responder, e é qualitativa, logo
não precisa de amostra grande.

\textbf{Duas salvaguardas contra mim próprio.} O limiar de oito participantes para o teste
estatístico está fixado no código \textbf{antes de existirem dados}, portanto baixá-lo apareceria
no histórico de alterações em vez de passar despercebido. E o procedimento de análise, corrido
sobre a folha por preencher, responde que está vazia e não produz resultado nenhum.

\textbf{E um bloco que foi retirado, por medição e não por conveniência.} O desenho original tinha
um terceiro bloco sobre o texto gerado. As rotas que o serviam foram retiradas do produto, o que se
confirmou a correr o procedimento de captura contra o sistema em produção, e não a supor. Medir a
utilidade de uma funcionalidade que o produto entregue não tem não faria sentido. O bloco fica
escrito e marcado como \emph{não correr}, porque apagá-lo esconderia que a pergunta existe.
