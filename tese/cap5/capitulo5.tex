...
\begin{tikzpicture}[
  font=\small,
  % altura fixa nas quatro, para a grelha ser mesmo uma grelha
  cel/.style={draw, minimum width=4.4cm, minimum height=1.75cm, align=center, inner sep=3pt,
              text depth=0pt},
  bom/.style={cel, fill=black!7},
  mau/.style={cel, fill=orange!13},
  rot/.style={font=\footnotesize\bfseries, text=black!65},
]
  % ️ Posições ABSOLUTAS, e não `right=0mm of`. Duas das células têm uma linha de texto a mais
  % (o "(alarme falso)" e o "(escapou)"), portanto crescem em altura; posicionadas umas em
  % relação às outras, a grelha partia-se e as quatro caixas saíam desencontradas. Numa figura
  % que existe para ensinar um 2x2, isso desfaz o próprio argumento. Só se vê a renderizar.
  \node[bom, anchor=north west] (vp) at (0,0)      {\textbf{Verdadeiro positivo (VP)}\\[2pt]
    \footnotesize assinalou, e o dia\\ \footnotesize \emph{era} mesmo invulgar};
  \node[mau, anchor=north west] (fp) at (4.4,0)       {\textbf{Falso positivo (FP)}\\[2pt]
    \footnotesize assinalou, e o dia\\ \footnotesize era banal\\ \footnotesize (\emph{alarme falso})};
  \node[mau, anchor=north west] (fn) at (0,-1.75)  {\textbf{Falso negativo (FN)}\\[2pt]
    \footnotesize calou-se, e o dia\\ \footnotesize era invulgar\\ \footnotesize (\emph{escapou})};
  \node[bom, anchor=north west] (vn) at (4.4,-1.75) {\textbf{Verdadeiro negativo (VN)}\\[2pt]
...
O mapa canónico do material histórico tem quinze nomes e o treino cobre catorze; a lista que o
...
\begin{tcolorbox}[colback=orange!5, colframe=orange!45, boxrule=0.5pt, arc=2pt]
Em \textbf{$84\%$} das decisões registadas, o resultado estava determinado pela
...
\paragraph{E a frase que eu tinha escrito aqui também não era exata.}
Durante várias versões deste capítulo esta lista dizia que a variante implantada foi escolhida
\emph{por ter a melhor PR-AUC entre as que cabiam no contentor}. A tabela desta secção desmente-o
quatro linhas acima: a variante só-volatilidade obtém $0.542$ e a variante sem os indicadores de
setor obtém $0.543$, contra os $0.538$ da implantada, e nenhuma delas precisa do codificador de
frases, portanto ambas cabiam. A justificação que eu dava a uma escolha errada estava ela própria
errada, dentro do parágrafo que existe para a corrigir.

A razão defensável que a escolha tem, e que só reconheço agora, é outra e não é de exatidão: a
variante de nove entradas é a única das três cujas contribuições o alerta consegue \emph{mostrar}.
A linha da Figura~\ref{fig:sis_alerta} que diz \emph{``raised by recent volatility (20d) and
sector; lowered by today's own move and recent momentum (5d)''} precisa de mais do que uma entrada
para existir, e com um modelo de uma variável reduzir-se-ia a repetir o nome dessa variável. Num
trabalho cuja restrição fundadora é a explicabilidade, isso é uma razão; mas é uma razão que
encontrei depois, e o preço em PR-AUC dela são os $0.005$ que a tabela mostra.
...
\paragraph{O resultado, e é positivo.} O título \textbf{acrescenta} a essa base:
...
\paragraph{E vale a pena dizer porque é que perdeu, porque a razão é de método e não de domínio.}
O que foi medido foi o FinBERT com a \textbf{média dos seus vetores} a servir de vetor de frase, que
é a única forma de o usar nesta tarefa sem o treinar de novo. Ora o FinBERT é um codificador afinado
...
Isto \textbf{estreita} a afirmação em vez de a reforçar, e é assim que fica escrita. O que a medição
mostra é que \emph{este} modelo de domínio, usado \emph{desta} forma, perde para um modelo genérico
treinado para similaridade. Não mostra que conhecimento de domínio não sirva para nada. Pô-lo à
prova a sério exigiria afinar um codificador financeiro com um objetivo de similaridade de frases e
voltar a medir, o que não foi feito e fica como trabalho por fazer.
...
pena mostrar, e o sistema escolhe-as melhor do que aquilo que já existe sem custo?
...
texto acrescenta} ao que o modelo já sabe da empresa: $+0.012$, IC $[+0.004, +0.020]$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{E um critério que este capítulo usa e nunca tinha enunciado.}
Vão aparecer diferenças pequenas lidas de maneiras diferentes, e convém dizer a regra antes de
elas surgirem em vez de a deixar adivinhar. Uma diferença de PR-AUC \textbf{abaixo de $0.02$} é
tratada aqui como indistinguível para efeitos práticos, mesmo quando o intervalo emparelhado
exclui zero. É por isso que o $+0.012$ do texto sobre a tabela de consulta é reportado como real
e ao mesmo tempo como incapaz de reabrir o veredicto, e que o $0.004$ entre a tabela de consulta
e o modelo implantado não sustenta escolher um em vez do outro. O limite é meu e é arbitrário;
o que não pode ser arbitrário é aplicá-lo nos dois sentidos, e é para isso que fica escrito.
