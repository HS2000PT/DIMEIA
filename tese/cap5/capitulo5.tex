...
Há uma pergunta que a comparação de famílias não faz, e que só me ocorreu depois de a ablação da
identidade explicar o que o modelo é.

Toda a avaliação da \gls{QI}3 põe as variantes \textbf{lado a lado}: só volatilidade contra só
contexto contra só texto. Uma comparação dessas responde a \emph{qual é melhor}. Não responde a
\emph{o texto acrescenta ao melhor que existe}, que é uma pergunta diferente e, para um sistema,
mais útil: ninguém é obrigado a escolher entre as duas fontes de informação.
...
o melhor preditor conhecido. O melhor é a tabela de consulta por empresa. Nunca se tinha testado
o texto por cima dela.
...
configuração que dá jeito.

\begin{table}[ht]
\caption[O texto por cima da melhor linha de base]{Somar o título à tabela de consulta. A coluna
da direita é a capacidade de ordenar \emph{dentro} de cada empresa, onde a tabela vale $0.5$ por
construção e não por estimativa.}
...
\paragraph{O resultado, e é positivo.} O título \textbf{acrescenta} à melhor linha de base:
$+0.012$ de PR-AUC, com intervalo $[+0.004,\ +0.020]$, que exclui zero. É o valor mais alto
medido em qualquer variante deste trabalho, e é a primeira vez que uma representação de texto
mostra valor mensurável nesta tarefa.
...
verdade. Acrescenta-se agora: \emph{mas o texto não é inútil}, acrescenta ao melhor preditor
conhecido, de forma pequena e mensurável, e o que o impede de mudar o sistema não é a ausência de
informação, é a informação ser do tipo errado. Distingue empresas quando o produto precisa de
distinguir notícias.

É uma resposta melhor do que a anterior porque diz \emph{onde} está o limite. E aponta o trabalho
seguinte sem ambiguidade: não é procurar um modelo maior, é arranjar um rótulo por notícia.
...
\textbf{QI3} triagem & \textbf{Não} & Nenhum modelo com texto bate a volatilidade de forma
sustentada ($0.496$ contra $0.542$; a melhor variante com texto chega a $0.547$, mas a diferença
tem intervalo que contém zero), e o resultado aguenta os três testes de robustez. \textbf{Mas o
texto acrescenta} ao melhor preditor conhecido: $+0.012$, IC $[+0.004, +0.020]$ \\
\bottomrule
\end{tabular}
\end{table}
