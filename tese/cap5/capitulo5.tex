...
ponto se separa por cortes aleatórios, o \emph{Local Outlier Factor} pela densidade em torno de cada
ponto \autocite{chandola2009anomaly}. Essa definição é uma vantagem quando há muitas variáveis a
interagir, e aqui não há: há uma série de retornos por empresa, cuja estrutura relevante é
\emph{temporal}. O agrupamento de volatilidade \autocite{engle1982arch, bollerslev1986garch} faz com
...
``movimento extremo'' e portanto é possível contar acertos e falhas. Para a recuperação existe uma
noção verificável de relevância e portanto existe precisão@$k$. Para a triagem existe um rótulo
construído a partir de preços futuros. Em todos estes casos há um alvo externo.
...
perguntar o que, nela, é falsificável. Há duas coisas.
...
aritmeticamente correta. Medido sobre as 17 empresas num mesmo dia: o motor identificado foi a
...
estimação. O $R^2$ mediano foi $0.460$. \textbf{Numa das 17 empresas foi negativo}, o que significa
que, naquela janela, mercado e setor não explicavam a variação daquela ação melhor do que a sua
própria média.
...
Convém dizer o que a linha ``palavras em comum'' representa, porque não é um adversário fraco
montado para perder. A recuperação por sobreposição de termos é a tradição que vai do modelo de
espaço vetorial \autocite{salton1975vsm} ao BM25 \autocite{robertson2009bm25}, e continua a ser uma
linha de base forte em recuperação de informação. Que a representação de frases
\autocite{reimers2019sbert} a supere em $17$ pontos percentuais é, por isso, um resultado com
conteúdo, e não a comparação com um espantalho.
...
\autocite{friedman2001gbm}, incluído precisamente por ser o aprendiz não linear mais forte
disponível para dados desta natureza: serve de teto, e se o texto tivesse sinal a extrair era ali
que apareceria. Segundo, todas as pontuações são convertidas em probabilidades por calibração
...
\subsection{E o que é que o portão estava a escolher, afinal?}
\label{sec:av_portao}
...
A pergunta é simples. O portão implantado era um limiar fixo sobre a probabilidade calibrada: passa
quem estiver acima de $0.50$. \emph{Esse limiar separava manchetes, ou separava empresas?}
...
\begin{tikzpicture}[font=\footnotesize, x=13cm, y=0.44cm]
  ...
  {
    \node[left, black!75] at (0.22,\i) {\t};
    \draw[line width=2.6pt, color=\c] (\a,\i) -- (\b,\i);
    \fill[black!85] (\m,\i) circle (1.1pt);
  }
...
\end{tikzpicture}
\caption[O que o portão separava]{Para cada empresa, o intervalo de todas as pontuações que o
...
estava tomada antes de se ler a manchete.}
\label{fig:av_portao}
\end{figure}
...
\begin{tcolorbox}[colback=orange!5, colframe=orange!45, boxrule=0.5pt, arc=2pt]
Em \textbf{$84\%$} das decisões, o resultado estava determinado pela \textbf{empresa} antes de a
manchete ser lida. A Apple não conseguia gerar um alerta de notícia, acontecesse o que
...
sua entrada manchetes que já passaram relevância e frescura. Nenhum teste podia detetar isto, porque
...
mostrou, das suas entradas apenas uma distingue duas manchetes da mesma empresa no mesmo dia: o
comprimento do título. Com o peso que essa entrada tem, oitenta caracteres a mais deslocam a
pontuação em cerca de $0.013$, contra uma amplitude de $0.385$ entre empresas.
...
é a fração das suas notícias que se revelaram importantes. Essa tabela não vê a manchete, não vê o
...
$\mathbf{0.632}$ \\
\midrule
Sem os indicadores de setor & tira 5 entradas de empresa & $0.543$ & $0.632$ \\
Sem volatilidade nem momento & tira as 2 que restam & $0.389$ & $0.474$ \\
Sem nada de nível de empresa & fica o dia e a notícia & $0.378$ & $0.164$ \\
Só o comprimento do título & a única de nível de notícia & $0.378$ & $0.164$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{A tabela de consulta iguala o modelo.}
Obtém $0.534$ contra $0.538$, uma diferença de $0.004$, e exatamente a mesma precisão dentro do
orçamento. Um número por empresa, fixado meses antes e nunca actualizado, faz o mesmo que o modelo
treinado. \textbf{O modelo implantado é uma tabela de consulta.}
...
O comprimento do título, sozinho, também dá $0.378$. É o mesmo chão. A única informação por
manchete que o modelo implantado recebia é indistinguível de não receber informação nenhuma.
...
\begin{itemize}
  \item \textbf{A conclusão científica da \gls{QI}3 mantém-se.} A variante \emph{com} texto tem
        $384$ números por manchete, portanto tem mesmo informação sobre a notícia, e ainda assim
  ...
  \item \textbf{A decisão de produto estava errada por outra razão.} O que foi implantado como
        ...
        pergunta que o produto precisava.
\end{itemize}
...
Dentro de uma empresa, a pontuação do modelo quase não varia com a manchete. Nenhuma regra de
decisão aplicada a essa pontuação a pode tornar sensível à notícia, porque a informação que
distinguiria uma manchete da seguinte não está lá.
\end{tcolorbox}
...
notícias por dia. As duas últimas linhas não são políticas: uma é o chão e a outra é o tecto.}
...
Volatilidade da empresa & doze constantes, sem ler manchete & $\mathbf{0.662}$ \\
\midrule
...
Doze constantes calculadas só sobre o bloco de treino obtêm $0.662$. Já se sabia da
