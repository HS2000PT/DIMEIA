...
O sistema chama-se \textbf{InvestiGator} e tem cinco partes, na Figura~\ref{fig:arquitetura}. Cada
uma faz uma coisa só, e passa o resultado à seguinte.
...
\begin{tikzpicture}[
  ...
  \node[proc, right=1.2cm of p] (det) {\textbf{Deteção}\\ \footnotesize é invulgar?};
  \node[proc, right=1.2cm of n] (rec) {\textbf{Recuperação}\\ \footnotesize já aconteceu?};
  \node[proc, right=1.2cm of h] (tri) {\textbf{Triagem}\\ \footnotesize merece avisar?};

  \node[proc, right=1.3cm of rec, text width=2.8cm] (exp)
    {\textbf{Explicação}\\ \footnotesize monta o texto a\\ \footnotesize partir do que foi\\ \footnotesize calculado};

  \node[saida, right=1.2cm of exp, yshift=9mm] (tg) {\textbf{Telegram}\\ \footnotesize alerta no telemóvel};
  \node[saida, right=1.2cm of exp, yshift=-9mm] (web) {\textbf{Página web}\\ \footnotesize consultar quando\\ \footnotesize apetecer};
