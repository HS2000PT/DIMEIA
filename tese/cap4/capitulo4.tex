...
São as $5\,060$ avaliações que o registo continha quando foi lido, e não o dia inteiro: o ficheiro
tem um limite de linhas, pela razão explicada na Secção~\ref{sec:met_dados}. O que se lê aqui são
proporções, não totais. A coluna da direita é a parte que interessa.}
\label{tab:sis_funil}
\centering
\small
...
são ordenadas pela pontuação e só as cinco melhores são enviadas. Substituiu um teto de duas por
empresa, que limitava cada nome e não o total. \\
\bottomrule
\end{tabular}
\end{table}

O chão de semelhança merece um parágrafo próprio, porque é a porta que trava mais casos e a que
tem a justificação mais fraca. Numa varredura medida, sete das dez empresas foram travadas exatamente
