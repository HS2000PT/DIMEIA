...
\end{columns}
\vspace{1mm}\par\centering\footnotesize No FNSPID multi-ano, sob a restrição causal da produção:
\textbf{P@5 $0.513$ vs chão $0.259$} (margem $+0.254$). O teste simétrico de escala dá $0.595$;
consistência de direção $0.71$ no chão $0.69$ (tema, não direção).
\end{frame}
...
\midrule
Ordenação aleatória (base) & 0.378 & 0.379 \\
...
\small Dentro de um orçamento de 5 alertas/dia, a triagem sobe a precisão \textbf{$1.67\times$} ($0.632$ vs $0.379$) \emph{em dados retidos do corpus de treino} --- mas \textbf{nenhum modelo de texto bateu a volatilidade}. Uma ablação de seguimento juntou \emph{cinco} features de contexto baratas (regime de mercado, momentum, reação padronizada, risco de queda): PR-AUC $0.535$, de novo \textbf{sem ganho}. Um re-teste \emph{justo} (C afinado, texto reduzido por PCA, um encoder de domínio FinBERT) continua a não bater a volatilidade, pelo que o negativo é \textbf{robusto}, não um artefacto de sub-ajuste. O sinal é o contexto de mercado; reportado tal como se revelou.
\end{block}
\end{frame}
...
precisão de $0.379$ para $\mathbf{0.632}$ num orçamento de 5 alertas --- e \textbf{o tecto nunca a
...
longo do dia a ordem continuou a ser a de chegada. O total é governado pelo \textbf{orçamento
global de 5/dia}. Com ele ligado, o primeiro alerta de cada empresa fica sem piso e o segundo
exige $0.64$; o $0.49$ permanece apenas como proveniência do varrimento que derivou ambos.}
\end{columns}
\vfill
...
\end{block}
\smallskip
{\footnotesize Medido: \textbf{ROC-AUC $0.486$}, IC de cluster $[0.403, 0.571]$ sobre $239$
...
materialidade nas decisões registadas corre a $0.602$ contra $0.378$ no treino, porque só se
...
\smallskip
$^{*}$\,Amostrado de uma corrida de seis relatórios: a \emph{taxa} transita, as contagens não. As
duas primeiras linhas são determinísticas e reproduzem exactamente; o zero tem de valer em
\emph{todas} as corridas.}
...
\begin{itemize}
  ...
  \item O corpus recente é uma janela \textbf{curta}; no FNSPID multi-ano a tarefa causal dá P@5 $0.513$ contra chão $0.259$. O $0.595$ vem do protocolo simétrico de escala, que permite candidatos posteriores.
...
\begin{itemize}
  ...
  \item \textbf{RQ2 (sim):} sob a restrição causal da produção, P@5 $0.513$ contra chão $0.259$ (margem $+0.254$); $0.595$ é o teste simétrico de escala. Impacto medido sem lookahead.
  ...
  \item \textbf{RQ4 (não no texto; ganho offline no proxy):} a triagem melhora $1.67\times$ a precisão dentro do orçamento ($0.632$ vs $0.379$), como limite superior da política online; a volatilidade ficou imbatida. Em produção, o mesmo gate ordena ao acaso (Resultado 9).
\end{itemize}
\vfill
...
\begin{itemize}
  ...
  \item \textbf{O vosso modelo treinado perdeu para a linha de base da volatilidade. Fracasso?} Não: a comparação foi \emph{pré-comprometida}. Offline, a triagem melhora $1.67\times$ o proxy no orçamento, mas é um limite superior e não utilidade humana.
  \item \textbf{Então o ML funciona em produção?} \emph{Como selector não, e medi-o} (ROC-AUC $0.486$). Como controlo de volume sim. Os filtros a montante tornam-no redundante na população implantada.
  ...
  \item \textbf{Como sabem que o modelo de triagem nunca vê o futuro?} No referencial diário, um teste muta preços posteriores a $d$; as entradas não mudam e o rótulo muda. Limite: \texttt{ret\_event} só fica completo no fecho e não está disponível quando a notícia chega intradia.
