...
\end{block}
\smallskip
{\footnotesize Measured: \textbf{ROC-AUC $0.486$}, cluster CI $[0.403, 0.571]$ over $239$
...
among logged decisions runs at $0.602$ against $0.378$ in training, because decisions are only
...
\begin{itemize}
  ...
  \item \textbf{RQ4 (no on text; offline proxy gain):} triage improves within-budget precision $1.67\times$ ($0.632$ vs $0.379$), as an upper bound on the online policy; volatility stayed unbeaten. In production the same gate ranks at chance (Result 9).
\end{itemize}
\vfill
...
\begin{itemize}
  ...
  \item \textbf{Your trained model lost to the volatility baseline. Failure?} No: the comparison was \emph{pre-committed}. Offline, triage improves the within-budget proxy $1.67\times$, but that is an upper bound and not measured human utility.
  \item \textbf{So does the ML work in production?} \emph{As a selector, no, and I measured it} (ROC-AUC $0.486$). As a volume control, yes. Upstream filters make it redundant in the deployed population.
  ...
  \item \textbf{How do you know the triage model never sees the future?} At the daily-close boundary, a test mutates prices after $d$; inputs stay fixed and the label changes. Limit: \texttt{ret\_event} is complete only at the close and unavailable when news arrives intraday.
