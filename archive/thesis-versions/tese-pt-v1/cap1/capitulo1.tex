...
A investigação em finanças comportamentais estabelece que as decisões destes investidores se
afastam de forma sistemática, e não aleatória, do ideal racional \autocite{barberis2003behavioral}.
Dois desvios são particularmente relevantes para o problema tratado nesta dissertação. O primeiro
é a aversão à perda: perdas são percecionadas com maior intensidade do que ganhos equivalentes
\autocite{kahneman1979prospect}, o que aumenta o custo da ausência de contexto precisamente quando
a informação é desfavorável. O segundo é o efeito da atenção: perante um número elevado de
alternativas, o investidor particular tende a adquirir os ativos que lhe captam a atenção,
identificados por três indicadores observáveis, que são a presença em notícias, o volume de
transação anormal e o retorno extremo num único dia \autocite{barber2008glitters}. Os autores
demonstram ainda que este padrão de aquisição não gera retornos superiores.

A confluência destes dois resultados delimita o problema. Os sinais que captam a atenção do
...
investidor já ter sido tomada. A segunda é teórica e é desenvolvida no Capítulo~\ref{cap:contexto}:
a hipótese dos mercados eficientes, na sua forma semiforte, sustenta que os preços refletem já a
informação publicamente disponível \autocite{fama1970efficient}, o que torna improvável a
antecipação sistemática de movimentos a partir dessa informação. Esta dissertação não testa essa
hipótese nem se apoia nela para estabelecer qualquer conclusão.
...
\begin{enumerate}
  ...
  \item \textbf{Um método reprodutível para medir o comportamento do preço na sequência de
        notícias semanticamente próximas}, combinando recuperação por semelhança de frases
        \autocite{reimers2019sbert} com janelas de estudo de evento na tradição de
        \textcite{brown1985daily}, sem utilização de informação posterior ao momento da decisão.
