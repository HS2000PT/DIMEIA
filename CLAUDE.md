...
## Estado Atual
- **Sessão nº:** 21 (MASTER PLAN A–H: **Fases A–G concluídas** nesta sessão; **só falta a Fase H — caderno de defesa visual**)
...
- **FASE G CONCLUÍDA:** `slides/` — slides de defesa (Beamer, 14 frames) destilados da tese validada; compila 0 erros; reutiliza figuras validadas; último frame = perguntas antecipadas do júri.
- **PRÓXIMO (FASE H — caderno de defesa visual, último do plano):** melhorar `docs/defence/caderno_de_defesa.md` (PT-PT) para o aluno estudar e **visualizar o workflow de dados/passos**: diagramas/explicações passo-a-passo, exemplos reais (TSLA z=7.61; recuperação Nvidia cross-ticker), "como explico ao júri em 3 frases" por componente, e mapa dos números validados. **Humano (única pendência de submissão da tese):** confirmar redação ISEP da declaração de IA + data; leitura final do aluno (§6.6).
