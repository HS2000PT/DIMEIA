...
- **FASE A em curso (conteúdo+visuais → ~80 pp).** **Concluído:** A1 3 algoritmos (Lista de Algoritmos preenchida) · A2 figura do fluxo mestre de dados/passos (Cap. 4) · A3 figura conceito de embeddings + linha temporal do event study (Cap. 3) · A4 exemplos trabalhados (z-score hipotético no Cap. 3; **recuperação real reproduzível** sobre a KB-amostra no Cap. 3 — query Nvidia → 3 precedentes AI, match cross-ticker MSFT, impacto médio +5d=+6.5%; **anomalia real** TSLA 24-10-2024 z=+7.61 no Cap. 5) · A5 Lista de Código removida. **+ Cap. 2 §2.7 "Existing Tools for the Retail Investor"** (vs alertas de corretora / apps de sentimento / robo-advisors; tabela; 2 citações novas verificadas: `dacunto2019robo`, `cardillo2024robo`). **+ Cap. 5 "Threats to Validity"** reescrito pela taxonomia (construct/internal/external/statistical-conclusion). **Estado: compila 70 pp, 0 erros, 0 citações indefinidas, 0 overfull >15pt; 42 refs; 41 testes verdes + ruff limpo.**
- **REALIDADE DA CONTAGEM DE PÁGINAS (medida, não suposição):** das 70 pp físicas, **16 são páginas em branco** (versos do `twoside`/`openright`, sobretudo no front matter: pp. 2,4,6,8,10,12,16,18,20,22,24,28 + versos de capítulo 52,64,68) + ~5 esparsas → **conteúdo real ≈ 53 pp**. O documento é **muito denso em floats**: cada acréscimo de prosa/figura é absorvido (re-empacotamento) e só "transborda" em páginas novas quando se acumula o suficiente. Por isso 68→70 apesar de ~5 pp de conteúdo novo esta sessão. **Chegar a ~80 exige mais prosa genuína ao longo de várias sessões da Fase A — SEM encher** (o aluno proíbe enchimento). Conclusões/Introdução já estão completas (aprofundá-las seria encher). **PRÓXIMO:** continuar Fase A com conteúdo genuíno (candidatos: diagrama de sequência por gatilho [o aluno quer "visualizar o workflow"]; aprofundar 1–2 áreas do Estado da Arte com fontes verificadas; `docs/design/how_to_run.md` [também Fase D]) **ou** seguir para Fase B (naturalidade) se o aluno aceitar ~70 pp densas. **Humano (em paralelo):** confirmar redação ISEP da declaração de IA + data; ler/rever a tese (§6.6).
...
> Conclusions), **42 referências verificadas**, **compila 70 pp, 0 erros, 0 citações indefinidas, 0 overfull >15pt**;
