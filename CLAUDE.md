...
- **🆕 SESSÃO 68 (2026-09-10): O REPOSITÓRIO REORGANIZADO, E A RAIZ PASSOU A TER OITO
  ...
  **⚠️ (A) CAMINHOS QUE MUDARAM, E QUEM RETOMAR TEM DE SABER:**
  `ESTADO_ATUAL.md` e `TASKS.md` → **`docs/contexto/`**; `BRIEF_REESCRITA.md`,
  `PROMPT_TAREFA.md` e `ESTADO.md` saíram de dentro de `tese-pt/` para `docs/contexto/`
  (o último renomeado para **`ESTADO_REESCRITA.md`**, senão eram dois ficheiros a dizer «o
  estado é este»); `requirements-app/ml/lock` → **`config/requisitos/`**; `progress/` →
  **`archive/progress-historico/`**; 17 *one-shots* gastos → **`archive/scripts-gastos/`**.
  **Tudo com `git mv`** — o `git log --follow` sobrevive. 28 ficheiros de referência
  reescritos, **zero referências órfãs** depois de verificado.
  **⚠️ (B) A ÚNICA QUEBRA FUNCIONAL ERA UMA, e não estava onde eu esperava.** O
  `setup_env.sh` instalava `requirements-ml.txt` pelo caminho antigo. Os quatro *workflows*
  do CI estavam **limpos** — invocam `requirements.txt`, que fica na raiz por obrigação do
  *buildpack*.
  **✅ (C) A RAIZ: OITO SOLTOS, E OS OITO SÃO OBRIGATÓRIOS.** `Procfile`, `app.json`
  (Heroku lê-os na raiz), `requirements.txt` (é o que faz o *buildpack* detectar a
  aplicação), `pyproject.toml` (packaging + config do `pytest` e do `ruff`), `README.md`,
  `CITATION.cff` (convenções do GitHub), `CLAUDE.md`, `AGENTS.md` (contexto de agente).
  ⚠️ **E contei sete antes de contar oito.** A promessa de contagem estava escrita no
  `README.md` antes de eu somar a tabela que a acompanha — é a classe de defeito que as
  portas deste projecto apanham, e apanhou-me a mim.
  **✅ (D) O `ruff` FICOU LIMPO PELA PRIMEIRA VEZ EM VÁRIAS SESSÕES, e a causa é
  específica.** Os sete avisos arrastados eram todos dos `_patch_*.py` gastos. E mover o
  rascunho de 47 MB para dentro da árvore fez o `ruff` passar a varrê-lo: **61 erros**, 54
  deles meus por deslocação. Correcção certa: `extend-exclude = ["archive"]` — o arquivo
  não é código vivo, e uma porta que grita sempre é uma porta em que se deixa de olhar.
  **⚠️ (E) DUAS AFIRMAÇÕES DE ARRUMAÇÃO ESTAVAM FALSAS, e as duas mandavam proteger o que
  já não existe.** O `archive/README.md` dizia que **`progress/`** tinha quem o chamasse —
  `check_all_gates.py`, `make_public_bundle.py`, `ci.yml`. **Medido: nenhum dos três.** O
  único acerto de `grep progress` nos *workflows* é `cancel-in-progress`, que não é a
  pasta — a mesma classe de falso positivo que este projecto já pagou cinco vezes. E dizia
  que `thesis/` e `thesis-pt/` tinham de ficar; estão em `archive/thesis-versions/` desde
  há sessões.
  **⚠️ (F) E O PLANO DESTA MESMA TAREFA AUTODECLARAVA-SE «AINDA NÃO EXECUTADO» HÁ NOVE
  DIAS.** O `docs/design/reorganizacao.md` é de 2026-09-01 e **metade dele estava feita**
  (`tmp/` de 66 MB fora, `archive/thesis-versions/`, raiz de 17 para 8 `.md`). Um plano
  assim ao lado de trabalho feito **manda a sessão seguinte refazer tudo** — é o defeito
  que a sessão 61 pagou com o `achados_citacoes_por_consumir.md`. Ficou com registo de
  execução, item a item.
  **⏭️ (G) O QUE CONSCIENTEMENTE NÃO SE FEZ, E É PARA NÃO REABRIR.** A §3 desse plano
  propõe mover tudo para **`code/`** e **`dissertation/`**. Custo contado, não estimado:
  os **dezanove verificadores** fixam `tese-pt/` e `tese-eng/` no caminho; o `Procfile`
  passaria a depender de `PYTHONPATH`; os `latexmkrc` e o `graphicspath` mudam de raiz; os
  manifestos de `docs/design/*_manifest.json` guardam caminhos com `sha256`; o CI tem
  quatro *jobs* que os invocam. **O benefício é arrumação e o custo é risco**, e o júri
  recebe um PDF — não abre o repositório. Se for para fazer, é **depois da defesa**.
  **✅ (H) O BUNDLE PÚBLICO GANHOU DUAS EXCLUSÕES QUE DECORREM DA REORGANIZAÇÃO.** Excluía
  só `progress/`; agora exclui **`archive/` inteiro** (que passou a ser o destino de tudo o
  que não é entrega, incluindo relatórios internos e versões superadas da tese) e
  **`docs/contexto/`**. Verificado a correr: 755 copiados, **190 internos excluídos**, scan
  de segredos limpo, e nem `archive/` nem `docs/contexto/` entram.
  **PORTAS: 1174 testes · `ruff` limpo · `check_entrega` verde nos 20 verificadores · as
  duas árvores a 0 erros (PT 2 550 359 bytes · EN 2 500 715) · nenhum `.tex` tocado, logo a
  composição é idêntica.** Pendência única e humana: os nomes do júri.
...
- **✅ SESSÃO 40 (FECHO — "continue with the plan; i'm already on the best pc"):** o aluno estava
  ...
  **✅ F9 (commit 106ed97) — bundle público:** `scripts/make_public_bundle.py` (parte de `git ls-files`
  ⇒ nunca inclui `.env`/segredos/corpora; remove os caminhos só-internos: archive/progress-historico/, CLAUDE.md,
