...
- **PRÓXIMO (FASE B — naturalidade, em curso):** continuar a passagem de voz académica/natural capítulo a capítulo (feito: secções novas do SoTA; FALTA: rever Cap. 3 Métodos, Cap. 4 CLARION, Cap. 5 Case Studies e o resto do Cap. 2 — reduzir travessões/tics de IA, variar aberturas, frases curtas; aplicar o estilo do feedback do orientador). Depois Fase C (revisão crítica do zero). **Humano (em paralelo):** confirmar redação ISEP da declaração de IA + data; ler/rever a tese (§6.6).
