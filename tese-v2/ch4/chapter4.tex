...
\begin{tikzpicture}[
  ...
  \node[fonte] (p) {\textbf{Prices}\\ \footnotesize cadeia de quatro\\ \footnotesize alternativas};
  \node[fonte, below=6mm of p] (n) {\textbf{News}\\ \footnotesize três interfaces\\ \footnotesize gratuitas};
  \node[fonte, below=6mm of n] (h) {\textbf{Case base}\\ \footnotesize precedentes com\\ \footnotesize impacto medido};

  \node[proc, right=0.8cm of p] (det) {\textbf{Detection}\\ \footnotesize $z$-score};
  \node[proc, right=0.8cm of n] (rec) {\textbf{Retrieval}\\ \footnotesize vizinhos semânticos};
  ...
  \node[proc, right=0.9cm of rec, text width=2.8cm] (exp)
    {\textbf{Explanation}\\ \footnotesize composição a partir\\ \footnotesize dos objetos\\ \footnotesize calculados};

  \node[saida, right=0.8cm of exp, yshift=9mm] (tg) {\textbf{Messaging channel}};
  \node[saida, right=0.8cm of exp, yshift=-9mm] (web) {\textbf{Web application}\\ \footnotesize registo de decisões};
