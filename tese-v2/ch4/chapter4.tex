...
\begin{axis}[
  ig axis, xbar, width=0.72\textwidth, height=6.2cm,
  xmin=0, xmax=3450,
  bar width=9pt,
  enlarge y limits=0.11,
  symbolic y coords={passagens,orcamento,repetida,escada,triagem,precedente},
  ytick={passagens,orcamento,repetida,escada,triagem,precedente},
  yticklabels={{atravessou todas as portas},{orçamento diário esgotado},{história já comunicada},
               {piso escalonado},{abaixo do piso de triagem},{sem precedente suficiente}},
  xlabel={avaliações registadas (total: $5\,060$)},
  tick label style={font=\footnotesize},
  label style={font=\footnotesize},
...
\addplot[bar shift=0pt, ig alternative bar] coordinates {
  (21,orcamento) (249,repetida) (269,escada) (1194,triagem) (2994,precedente)};
\addplot[bar shift=0pt, ig reference bar] coordinates {(333,passagens)};
\end{axis}
\end{tikzpicture}
\par\smallskip
\fbox{\parbox{0.78\textwidth}{\centering\small
  \textbf{5 mensagens entregues}\quad$\ne$\quad\textbf{333 passagens pelas portas}\\
  Uma avaliação não é uma notícia distinta; os mesmos títulos eram reavaliados.}}
\caption[Distribuição completa das avaliações por ponto de decisão]{As $5\,060$ avaliações
registadas em parte de 15 de agosto de 2026. As seis categorias somam o total; a barra tracejada
inclui as $333$ passagens por todas as portas. As cinco mensagens entregues são uma contagem
separada, não uma sétima categoria de avaliações. O orçamento diário foi integralmente utilizado. A data é
...
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]},
  ...
  \draw[->, dashed, black!45] (q.south) -- ++(0,-0.55)
    node[below, align=center, black!60] {não supera:\\ regista e descarta};
\end{tikzpicture}
