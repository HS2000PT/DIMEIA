...
def main() -> int:
    ...
    if cache.exists():
        allemb = np.load(cache); print(f"Cache de embeddings: {cache.name}")
    else:
    ...
    for s in ["train", "val", "test"]:
        off[s] = allemb[i:i + len(parts[s])]; i += len(parts[s])
    ...
    def full(emb_tr, emb_va, emb_te, tag):
        xtr = np.hstack([ctx["train"], emb_tr]); xva = np.hstack([ctx["val"], emb_va])
    ...
    for d in [16, 32, 64]:
        ...
        full(pca.transform(off["train"]), pca.transform(off["val"]), pca.transform(off["test"]),
             f"Context+text MiniLM PCA-{d} (tuned)")

    if args.finbert:
        fbcache = Path(args.dataset).with_name("_cache_triage_finbert.npy")
        if fbcache.exists():
            fb = np.load(fbcache); print(f"Cache FinBERT: {fbcache.name}")
        else:
            from investigator.historical_kb.embedder import SbertEmbedder
        ...
        fo = {}; i = 0
        for s in ["train", "val", "test"]:
            fo[s] = fb[i:i + len(parts[s])]; i += len(parts[s])
        full(fo["train"], fo["val"], fo["test"], "Context+text FinBERT (tuned)")
        for d in [32]:
            pca = PCA(n_components=d, random_state=42).fit(fo["train"])
            full(pca.transform(fo["train"]), pca.transform(fo["val"]), pca.transform(fo["test"]),
    ...
    verdict = ("o texto SUPERA a volatilidade" if best_text[3] > vol_te
    ...
    nuance = (
        f"**Nuance honesta (o arguente tinha razão num ponto):** o texto cru de 384 dims (afinado) dá "
        f"{raw_full:.3f}, mas reduzi-lo por PCA recupera até **{best_text[3]:.3f}** ('{best_text[0]}') "
        "— ou seja, o número congelado (full 0,496) estava EM PARTE deprimido por dimensionalidade "
        "(384 dims a diluir 5 escalares de contexto). ")
    if best_text[3] <= vol_te:
        end = (f"MAS mesmo o melhor texto justo ({best_text[3]:.3f}) não supera a volatilidade "
        ...
        end = (f"E o melhor texto justo ({best_text[3]:.3f}) SUPERA a volatilidade ({vol_te:.3f}): o "
    ...
    L += [
        "",
        f"**Veredicto (reportado tal como cai):** com o C afinado, o bloco de texto reduzido por PCA"
