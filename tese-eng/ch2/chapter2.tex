...
the authors describe --- feedback trading on price, as opposed to the use of information about the
value of the firm --- is precisely what takes hold when the recipient is handed a number and
...
The class of system at issue here --- watching for events and notifying the retail investor --- has
...
of alerting systems for the retail investor, because they exist and are published; it has to be
stated as a difference in epistemic stance, between estimating what will happen and explaining what
...
\autocite{ancker2017alertfatigue}, and whose transposition to this domain is no longer necessary
now that a measurement made on retail investors exists \autocite{liu2023alerts}, discussed in
Section~\ref{sec:ctx_investidor}. The consequence is that the decision policy forms part of the
...
These cautions are general. On the concrete choice made by this dissertation --- to explain by
...
comparison with the inductive style --- which is the example-based style, that is, the one closest
...
components that would solve each part were evaluated in isolation, over reference corpora, never
as a system in continuous operation against real sources.

This last claim needs a qualification, and Section~\ref{sec:ctx_ferramentas} gave it a name.
Integrated systems for this audience exist and are published, the most developed being that of
\textcite{muntermann2009ubiquitous}. The difference from that work is not one of integration, which
it also achieved, but of two other things. The first is epistemic stance: that system estimates the
price effect that will follow the announcement, and this one measures the effect that has already
occurred and explains it. The second is the object of the evaluation: that one measures economic
opportunity by simulation, and this one measures the quality of what it delivers, against real data
and over a period of operation. Stated in this way, the gap is narrower than a claim of absence,
