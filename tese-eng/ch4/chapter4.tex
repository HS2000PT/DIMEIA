...
are generated from a snapshot of the application itself, and not collected by hand, with the
capture width fixed in the generator of each one, since it is on that width that the interface
text stays legible after being reduced to the text box of this document.}
...
distinction between flagging and communicating becomes observable. The italic line beneath the
split states the quality of the fit that produced it, compared against the median across the
seventeen companies of the sector map, and is what lets the reader know when the split is to be trusted.}
...
a rise of $0.99\%$, and the market and the sector were both pulling the other way, at $-0.47\%$ and
$-0.54\%$; the company-specific part was $+2.00\%$, more than twice the observed movement. The
model attributes the rise entirely to the company, and not to the environment in which it trades.
This statistical split does not rule out unmodelled external causes. The verdict classifies the
day as ordinary: $141$ of the last $250$ trading days recorded an equal or larger change.

The rarity count the card presents runs over one year of sessions, whereas the alert's runs
over the six months the scan holds at the moment it composes it. The denominator differs for
that reason between Figure~\ref{fig:intro_lacuna} and this one, for the same company on the
same day, and the fraction each expresses is close.

The same figure shows the caveat that accompanies it. The coefficient of determination for this
company is $0.26$, below the median of $0.46$ across the seventeen companies of the sector map, and the interface states this beneath the
split, with the instruction to read it as indicative. The two statements coexist by design: the
split is the best estimate available and, for this company, the market and the sector explain only
a quarter of the daily variation. Omitting the second would make the first more persuasive than
the fit that supports it, which is the defect that Section~\ref{sec:con_limitacoes} names.

Figure~\ref{fig:sis_silencio} shows the same requirement on the side of silence, on the day of
Figure~\ref{fig:sis_pagina}: the seven companies that generated no alert all stopped at the same gate: the daily budget of five
