...
\addlinespace
$r_m$, $\tilde{r}_s$ & Daily market return and sector return orthogonalised against the market & Eq.~\ref{eq:decomposicao} \\
...
\markboth{How to read this thesis}{How to read this thesis}

This document describes a system that answers three questions a retail investor asks, and measures
what each of those answers is worth. It runs to about a hundred pages of body text, and not every
reader is after the same thing. This page proposes four paths.

\begin{description}
  \item[If you want to know what the work concludes] read Chapter~\ref{cap:introducao}, which
    states the three questions, and then Section~\ref{sec:con_respostas}, where each one receives
    its answer with the reach delimited. That is about twelve pages, and
    Figure~\ref{fig:con_cadeia} summarises the path of each question in a single image.

  \item[If you want to know how it was measured] read Chapter~\ref{cap:metodologia}, which
    describes the four techniques and the temporal discipline that stops each one from using
    information that did not yet exist, and then Section~\ref{sec:av_desenho}, which fixes the
    metrics and, what matters most, the value each one takes in the absence of information.

  \item[If you want to know how the system works] read Chapter~\ref{cap:sistema}. The path of a
    news item is in Section~\ref{sec:sis_percurso}, the policy that decides silence in
    Section~\ref{sec:sis_politica}, and the layer that composes the delivered text in
    Section~\ref{sec:sis_explicacao}.

  \item[If you want to verify] go to Appendix~\ref{ap:reprodutibilidade}.
    Table~\ref{tab:ap_numeros} links every reported result to the section that produces it, and
    Table~\ref{tab:ap_matriz} lists every substantive claim with the state it was left in after
    being measured, including those that were withdrawn.
\end{description}

Three conventions run through the document and are worth stating before you start. No number was
written by hand: each one is produced by an automatic procedure and the text cites the file that
contains it. No comparison is presented without the value a method with no information would
obtain, because a percentage read on its own leads to the wrong conclusion, and the document
records three occasions on which that nearly happened. And every result comes with what does
\emph{not} follow from it, which is the step that distinguishes an evaluation from a
demonstration.
