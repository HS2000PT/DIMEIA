...
the organisation of the document. Whoever reads it will know what problem the work solves, for whom, and what the system refuses to do by decision rather than by limitation.
...
Figure~\ref{fig:intro_lacuna} shows the difference on a concrete event.
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  % left box: 0.00 to 4.10 · right box: 4.70 to 13.70
  ...
  \node[tit] at (2.05,3.16) {what the percentage says};
  \node[tit] at (9.2,3.16) {what the system delivers, on the same day};

  \node[font=\large\bfseries] at (2.05,2.32) {META $+1.00\%$};
  \node[font=\scriptsize, align=left, text=black!62, anchor=west] at (0.42,1.15)
       {is it unusual?\\[3pt] was it the company?\\[3pt] has it happened before?};
  \node[font=\Large, text=black!30] at (3.65,1.15) {?};

  \node[q] at (5.00,2.35)
       {\textbf{is it unusual?} no: $73$ of the last $126$ days\\ moved at least as much};
  \node[q] at (5.00,1.35)
       {\textbf{was it the company?} the split of the move\\ into market, sector and company};
  \node[q] at (5.00,0.35)
       {\textbf{has it happened before?} $3$ similar cases, with a\\ five-day move from $+2.38\%$ to $+14.81\%$};
  \node[onde] at (13.42,2.35) {in the alert};
  \node[onde] at (13.42,1.35) {on the page};
  \node[onde] at (13.42,0.35) {in the alert};
\end{tikzpicture}
\caption[The same event, with and without context]{The same day of the same company, on the
left as a price quote presents it and on the right as the system delivers it. The values on
the right are reproduced from the alert sent to the channel on 8 September 2026, and were not
constructed for the purpose. The third column indicates where each answer lives, and the
distinction is not incidental: the alert answers the first and the third question at the moment
it interrupts the reader, whereas the split depends on the day's close and is therefore
presented in the application, as Figure~\ref{fig:sis_empresa} shows.}
...
Four research questions follow from this objective, each associated with a measurement and a
...
\begin{itemize}
  ...
  \item \textbf{\gls{QI}2, precedents.} Can the system retrieve news from another company in
        the same sector better than the evaluated textual and ranking alternatives, and retain
        an advantage over chance when only news prior to the query is allowed?
  ...
  \item \textbf{\gls{QI}4, comparability.} Can contrastive fine-tuning of the encoder, trained to
        bring together news items whose subsequent movement has similar \emph{magnitude}, improve
        the materiality comparability of the precedents retrieved without a cost in thematic
        relevance?
\end{itemize}

The fourth question calls for a justification, because it touches the founding constraint of this
work. The system does not predict, and training an encoder on observed outcomes might appear to
contradict that constraint. The decisive distinction is between \emph{direction} and
\emph{magnitude}: what the constraint forbids is asserting where the price will go, and the
training objective of this question is the magnitude of the movement, taken in absolute value.
What an encoder adjusted in this way produces is not an assertion about the news item under
analysis, but an organisation of the case base that brings together news items of comparable
materiality. So that the distinction is verifiable and not merely declared, the experiment
includes a second arm, trained on the \emph{signed} return: were direction as learnable as
magnitude, the two arms would be indistinguishable and the argument would fall.
...
\begin{enumerate}
  ...
  \item to evaluate the components, with transparent baselines for detection, retrieval and triage;
...
The techniques these objectives call for come from different families, and the question of
where machine learning sits in the work has a distinct answer for each of the five decisions
the system takes before an alert reaches anyone. One of them is a rule with no estimated
parameters. One uses no parameters beyond the mean and the deviation of the preceding window.
One has its parameters estimated by regression, with shrinkage. One relies on a model
pre-trained by others and used without adjustment. Only the last has parameters estimated in
this work, on the historical set. The label of artificial intelligence conflates these
situations, and Section~\ref{sec:met_onde_ia} separates them decision by decision, together with
what learning each one from data would require.
...
techniques until they constitute a working, explainable and reproducible system. Five
...
\begin{enumerate}
  ...
  \item \textbf{An evaluation of the components, with baselines for three of the four techniques}, including the
        result that contradicts the initial hypothesis, reported as such.
  \item \textbf{A negative result on learning materiality from text}, obtained with a protocol
        fixed before execution and with a diagnostic that rejects degenerate representations
        before any metric is interpreted. The first attempt produced exactly that degeneration,
        and the diagnostic caught it; Chapter~\ref{cap:avaliacao} reports both.
\end{enumerate}
...
system, and not on examples constructed for the purpose. One of the twelve companies, AMD,
returns in three distinct roles across the document, and serves as a thread for anyone who
wants to follow a single name: it is the movement that Section~\ref{sec:met_decomposicao} splits
into its three components, it is the company the decision gate let through in every logged
assessment, and it is one of those the model scores without ever having observed them in
training. These are three moments of the same company, not of the same event.
