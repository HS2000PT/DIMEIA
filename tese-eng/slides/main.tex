...
\midrule
\textbf{By meaning (the method)} & $\mathbf{0.395}$ \\
The most recent & $0.204$ \\
Always the largest sector & $0.200$ \\
By shared words & $0.196$ \\
At random (the floor) & $0.117$ \\
\bottomrule
\end{tabular}
\vspace{2mm}
...
\begin{itemize}
  \small
  \item The full corpus is 84\% technology, and there \textbf{always returning technology} is
        worth $0.840$ with no model at all, above any method. That is why the primary
        measurement uses the \textbf{balanced} subset, where the same strategy is worth $0.200$
        the method clears it comfortably, and the margin over chance is $+0.278$.
  \item The method beats chance in all \textbf{five sectors}: $0.589$ in energy, with a floor
        of $0.110$. No advantage over a \textbf{filter using the query's known sector} was measured.
  \item Requiring earlier candidates: \textbf{0.513}, floor \textbf{0.259}, margin
        \textbf{$+0.254$}. The test does not impose the eight-day maturation used in production.
  \item \textbf{Similarity of theme does not guarantee direction}. Cases are shown one by one.
\end{itemize}
\end{frame}
...
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]},
  ...
  \foreach \xn/\xlim/\xfut/\xcus in {%
    0/{No \emph{controlled} study with people}/{Run the study: 6 to 10 people}/{calendar},%
...
\begin{frame}{Did it help anyone? The one piece of evidence collected in the open}
Every alert carries two buttons. \textbf{The rules were fixed before the first vote existed}
and none was changed afterwards.
\vspace{2mm}
\begin{center}
\begin{tabular}{@{}lr@{}}
\toprule
Ratings, one per person per alert, over 62 alerts & \textbf{91} \\
People & \textbf{3} \\
\midrule
Called the alert useful & \textbf{86 of 91} $= 95\%$ \; {\scriptsize 88\% to 98\%} \\
The same, without the reader who gave $58\%$ of them & \textbf{34 of 38} $= 89\%$ \; {\scriptsize 76\% to 96\%} \\
\bottomrule
\end{tabular}
\\[1mm]
{\scriptsize at the right, the 95\% Wilson interval}
\end{center}
\vspace{1.5mm}
{\footnotesize \textbf{And what it is not.} Three people is a pilot, not a study. Whoever wants
to votes, which pushes the sample to the extremes; nobody received the move \emph{without} an
explanation, so nothing here credits the usefulness to the explanation itself; and perceived
usefulness is not a better decision. \textbf{The controlled study is designed and was not run} --
it is the first limitation of the conclusions.}
\end{frame}
...
{\footnotesize In words, before the number: $128$ of the last $248$ days moved
this much or more. The verdict is \emph{``Quiet''}, and it comes before any value.}
\vspace{3mm}

\textbf{2. Was it the company?}\\
{\footnotesize \textbf{No, it was not.} The stock falls $0.97\%$, and the company's own
part is \emph{positive}, $+0.11\%$: what pulled it down was the market ($-0.53\%$)
and the sector ($-0.55\%$). Reading the percentage alone gives the opposite answer.
And the line below says this company's fit sits \textbf{below} the median.}
\vspace{3mm}
...
{\footnotesize The decisions recorded for one company, each naming the headline and, where the
message was withheld, the reason. \textbf{It is the part a live demonstration cannot show}: nine
out of ten scans send nothing, and the silence is written down like an alert.}
