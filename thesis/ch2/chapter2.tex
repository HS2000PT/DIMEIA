...
that textual signals relate to market outcomes, which is the premise of InvestiGator's news engine. This
...
constraint that also shapes how InvestiGator is evaluated (Chapter~\ref{chap:casestudies}).
...
computation, a core criterion, and fidelity is the property InvestiGator can guarantee
...
\textbf{For InvestiGator:} Sentence-BERT is the sweet spot, contextual meaning beyond lexicons and static
...
supervised classification without breaking the no-forecast rule: estimate the probability that an
\emph{abnormal move of unusual size, in either direction}, follows a news item, where the label is
...
reacted at all, never the direction or magnitude of a price path, so the task sits deliberately on the
defensible side of the efficient-market line drawn earlier in this chapter.
...
contributions, which is precisely the kind of inherently interpretable model that
...
precision--recall curve rather than accuracy, together with a budget-shaped precision (how many of the
$N$ alerts a user can absorb per day were material), which measures exactly the alert-fatigue cost the
triage exists to reduce.
...
\textbf{For InvestiGator:} each existing tool covers one corner, alerting, news, or allocation, but none combines
