...
vectors, yet cheap, comparable and free to run. One honesty note on the comparison: the evaluation in
Chapter~\ref{chap:casestudies} measures Sentence-BERT against a \emph{lexical} baseline, which brackets
the question that matters (does meaning beat surface words?); word2vec and FinBERT variants are not run
empirically, because static word vectors are strictly dominated by contextual sentence embeddings for
sentence comparison, and domain-tuned FinBERT is oriented to sentiment classification rather than
sentence similarity, at a cost this work's free-and-reproducible constraint excludes. Where a claim is
cheap to test, this work tests it; where it would buy little, the reason is stated instead.
