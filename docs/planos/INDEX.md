...
written overview of the whole project, read **[`archive/reports/RELATORIO_FINAL.md`](archive/reports/RELATORIO_FINAL.md)**.
...
| **Graduated course, zero AI knowledge → defence (8 levels, phone, offline)** | [`archive/streamlit-app/study/index.html`](archive/streamlit-app/study/index.html) |
| Self-test app (48 questions, phone, works offline) | [`archive/streamlit-app/quiz/index.html`](archive/streamlit-app/quiz/index.html) — quick recall only; the course above is the one to learn from |
...
| Item | Where |
|------|-------|
| Windows one-click launchers | [`archive/streamlit-app/run/`](archive/streamlit-app/run/) |
| Always-on deployment (systemd units, VM setup) | [`archive/deploy/`](archive/deploy/) |
| End-to-end walkthrough notebook | [`archive/streamlit-app/notebooks/`](archive/streamlit-app/notebooks/) |
| Dashboard theme | [`.streamlit/config.toml`](.streamlit/config.toml) |
| What is still outstanding | [`docs/planos/CHECKLIST.md`](docs/planos/CHECKLIST.md) |
...
| Requirements (light / ML / app) | `requirements.txt`, `config/requisitos/requirements-ml.txt`, `config/requisitos/requirements-app.txt` |

> Large corpora, secrets and downloaded models are never versioned — they are regenerated by the
> data scripts. Working notes used while building the project live in `archive/progress-historico/` and `CLAUDE.md`.
