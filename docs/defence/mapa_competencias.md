...
| **Recuperação de informação** | Recuperação vetorial cross-ticker; teste por setor e protocolo causal que só admite precedentes anteriores | `correlation_engine/`, Caso 2 | P@5 causal **0.513** vs chão **0.259**; simétrica **0.595** vs **0.333** |
| **PLN** | Comparação de representações: léxico → estáticos → contextuais; benchmark de encoders medido, não argumentado | Cap. 2, `evaluation_retrieval_embedders.md` | MiniLM 0.395 > FinBERT 0.275 |
