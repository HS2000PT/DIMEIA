...
e sociais do domínio. Quem o leia fica a saber como cada uma das quatro decisões é tomada, e que garantia impede cada uma de utilizar informação que ainda não existia.
...
O primeiro consiste em comparar deteção, recuperação e triagem com alternativas mais simples,
selecionadas antes da medição, de modo que o resultado possa ser negativo. A decomposição recebe
apenas uma verificação descritiva da repartição e do ajuste. O segundo
...
autonomamente em produção. Esta segunda avaliação produz o
...
primeiras correspondem às três perguntas do investidor enunciadas no Capítulo~\ref{cap:introducao}; a quarta não é uma
...
\begin{tikzpicture}[
  ...
  \node[cx, right=0.6cm of a] (b) {\textbf{2. Empresa ou}\\\textbf{mercado?}\\[3pt]
    \footnotesize repartir o movimento\\ \footnotesize pelas suas origens};
  \node[cx, right=0.6cm of b] (c) {\textbf{3. Já aconteceu antes?}\\[3pt]
    \footnotesize recuperar casos passados\\ \footnotesize e medir o que se seguiu};
  \node[cx, right=0.6cm of c] (d) {\textbf{4. Justifica alerta?}\\[3pt]
...
A camada histórica utiliza a componente de notícias do \gls{FNSPID} \autocite{dong2024fnspid}, que
associa a cada título uma data e uma empresa. Embora o conjunto publicado também inclua preços,
neste trabalho os retornos e os rótulos são construídos alinhando os títulos com séries de preços
obtidas separadamente. Do alinhamento resulta o conjunto de treino do modelo de triagem, com
79\,753 exemplos.

O ficheiro de origem é identificado pela revisão, e não pelo nome. O \gls{FNSPID} é distribuído
num repositório cujo endereço por omissão aponta para o ramo principal, e um ramo é um ponteiro que
se move: um conjunto de dados assim referido não é reproduzível por construção. O corpus deste
trabalho parte do ficheiro \texttt{Stock\_news/nasdaq\_exteral\_data.csv} numa revisão fixa, com
23,2\,GB e soma de controlo SHA-256 de prefixo \texttt{1a7a3eb8}; a filtragem é local e
determinística, e o corpus que dela resulta tem prefixo \texttt{af61708c}. As somas completas, a
contagem por empresa e a auditoria do procedimento constam de um manifesto versionado no
repositório, e um teste automático recusa qualquer corpus que não seja este.

Duas decisões de construção ficam declaradas por afetarem a leitura dos resultados. A primeira é
que os títulos são conservados tal como o \gls{FNSPID} os entrega: o corpus contém 1\,704 títulos
exatamente repetidos, 2,1\% do total, que não foram removidos. A segunda é que a filtragem percorre
o ficheiro inteiro, em vez de parar quando julga ter passado os símbolos procurados; o ficheiro não
está globalmente ordenado por símbolo, e uma paragem antecipada truncaria o corpus em silêncio.

O conjunto cobre catorze das quinze empresas do mapa canónico. A ausente é a Meta, e a razão está
no conjunto de dados e não no procedimento: o \gls{FNSPID} indexa a empresa pelo símbolo antigo
\texttt{FB}, pelo que a seleção pelo símbolo atual não devolve registo nenhum. Os registos sob o
símbolo antigo existem, mas resumem-se a 432 títulos distribuídos por 87 dias entre fevereiro e
junho de 2020, quatro meses num intervalo de seis anos. A empresa consta, por isso, da avaliação
da deteção, que assenta em preços, e não do conjunto de notícias.
...
\begin{table}[!htbp]
\caption[Conjuntos de dados utilizados]{Conjuntos de dados utilizados no trabalho. O intervalo do
conjunto de treino da triagem refere-se a dias de negociação, depois do alinhamento: os títulos vão
de 2018-01-01 a 2023-12-16, e as duas datas extremas, um feriado e um sábado, são remetidas
para a sessão seguinte. As três últimas linhas correspondem ao sistema em funcionamento.}
...
produção contém duas empresas ausentes de todos os conjuntos históricos, e o mapa canónico contém
uma terceira, a Meta, que não chega ao conjunto de dados da triagem, pela razão dada acima.}
...
\section{Onde entra a inteligência artificial neste sistema}
\label{sec:met_onde_ia}

As quatro decisões que este capítulo apresenta a seguir recorrem a famílias de técnicas distintas, e a
pergunta sobre o lugar da aprendizagem automática neste trabalho tem uma resposta diferente
para cada uma. A Tabela~\ref{tab:met_onde_ia} reúne essa resposta, que de outro modo ficaria dispersa pelas
secções que se seguem, e acrescenta a decisão que as antecede a todas: o filtro que determina
quais os títulos que chegam a ser avaliados.

\begin{table}[!htbp]
\caption[Onde entra a aprendizagem, decisão a decisão]{As cinco decisões do sistema, a origem
dos respetivos parâmetros, e o que seria necessário para aprender cada uma a partir de dados.
A coluna central distingue três situações que a designação genérica de inteligência artificial
confunde: parâmetros estimados neste trabalho, parâmetros de um modelo treinado por terceiros e
utilizado sem ajuste, e ausência de parâmetros. Os valores da primeira linha são medidos e discutidos na
subsecção seguinte.}
\label{tab:met_onde_ia}
\centering
\small
\begin{tabular}{L{0.20\textwidth} L{0.24\textwidth} L{0.44\textwidth}}
\toprule
\tabhead{Decisão} & \tabhead{Origem dos parâmetros} & \tabhead{O que seria preciso para a
aprender} \\
\midrule
Relevância do título & regra; sem parâmetros estimados & rótulos independentes da própria
regra. O único disponível é a saída dela, o que torna o exercício circular \\
\addlinespace
Raridade do movimento & nenhum; média e desvio da janela anterior & rótulos de raridade que não
fossem eles próprios função da volatilidade \\
\addlinespace
Repartição do movimento & estimados por regressão, com encolhimento & já são estimados; o que
falta é um valor de referência observável para a repartição \\
\addlinespace
Comparabilidade de notícias & modelo pré-treinado por terceiros, utilizado sem ajuste & foi
tentado, e é a Secção~\ref{sec:met_qi4}; o resultado consta da Secção~\ref{sec:av_qi4} \\
\addlinespace
Materialidade da notícia & estimados neste trabalho, sobre o conjunto histórico & já são
estimados; o resultado consta da Secção~\ref{sec:av_qi3} \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{A decisão que antecede as outras, e é uma regra}
\label{sec:met_relevancia_nota}

Antes de qualquer das quatro, o sistema decide se um título diz respeito à empresa em causa. A
regra exige a menção da empresa, por nome ou por um dos seus nomes alternativos, e rejeita um
conjunto fechado de padrões de resumo de mercado. Medida sobre as dez empresas que a lista
vigiada percorre, retém $4\,843$ de $16\,840$ títulos, ou seja $28{,}8\%$, e descarta
$71{,}2\%$. A decomposição dos descartes localiza o trabalho: a regra da menção explica
$68{,}3\%$ do total e a lista de padrões apenas $3{,}0\%$.

É, em proporção, o filtro mais agressivo do sistema, e é o único que permanece uma regra. A
razão não é de conveniência. Aprender esta decisão exigiria títulos anotados quanto à sua
pertinência, e o único rótulo disponível em quantidade é a saída da própria regra, o que
tornaria o exercício circular. A alternativa honesta seria anotação manual, que a
Secção~\ref{sec:con_futuro} enuncia entre as linhas que exigem julgamento humano. Enquanto não
existir, a taxa de retenção depende da lista de nomes alternativos, e uma empresa sem entrada
nessa lista recai na exigência do símbolo, que quase nenhum título escreve.

\subsection*{O que isto significa para a leitura dos resultados}

As cinco decisões correspondem a cinco situações distintas, e a tabela distingue-as uma a
uma. A relevância é uma regra sem parâmetros. A raridade não tem parâmetros: compara o dia com a
média e o desvio da janela anterior. A repartição tem coeficientes estimados, mas estimados sem
rótulos, a partir dessa mesma série de retornos. A comparabilidade assenta num modelo treinado
por terceiros e utilizado sem ajuste. E só a materialidade tem parâmetros treinados sobre
exemplos rotulados, o que a torna a única decisão aprendida no sentido corrente do termo. O único lugar onde uma
representação foi treinada para este problema é o ajuste contrastivo do codificador, cuja
resposta consta do Capítulo~\ref{cap:avaliacao}.

Esta distribuição não é um acidente de execução, e é ela que dá sentido à comparação que
atravessa a avaliação. As duas decisões em que a aprendizagem deste trabalho foi aplicada, a
materialidade e a tentativa de ajustar o codificador, são também as duas cujas respostas são
negativas; as respostas afirmativas vêm de uma regra estatística e de um modelo utilizado como
está. A comparação entre cada técnica adotada e a alternativa aprendida correspondente é
apresentada em cada caso de estudo, e é a constatação com maior capacidade de transferência
deste trabalho. A designação
genérica de inteligência artificial descreveria mal um sistema em que a aprendizagem ocupa uma
das cinco decisões; nomear o lugar de cada família de técnicas é a forma de o descrever sem
prometer o que ele não faz.

\section{Deteção de movimentos invulgares: o \emph{z}-score deslizante}
...
\begin{tikzpicture}[font=\small, >={Stealth[length=2.2mm]}]
  ...
  \draw[decorate, decoration={brace, amplitude=5pt}, black!55]
    ...
    {\footnotesize os \textbf{20 dias anteriores}\\[-2pt]\footnotesize dão $\mu$ e $\sigma$};
  \draw[decorate, decoration={brace, amplitude=5pt}, orange!75!black]
    ...
    {\footnotesize o dia\\[-2pt]\footnotesize \textbf{a avaliar}};
...
A separação não é apenas uma convenção de escrita: é uma fatia. O Excerto~\ref{lst:janela}
reproduz o cálculo tal como é executado. A linha que a garante é a que estabelece a janela
\texttt{recent}, cujo intervalo termina no penúltimo elemento e exclui, por construção, o dia que
está a ser avaliado.

\begin{lstlisting}[caption={[A janela que exclui o dia avaliado]A janela que exclui o dia
avaliado. O intervalo \texttt{[-window-1:-1]} termina antes do último elemento, pelo que o
retorno do dia em avaliação não entra na média nem no desvio-padrão contra os quais é
comparado. Reproduz-se o corpo da função de deteção, sem a indentação que ela tem no ficheiro
de origem.}, label={lst:janela}, float=htbp]
...
desvio-padrão de $2{,}725\%$ nos vinte dias anteriores, o que corresponde a $z = +7{,}61$. O valor
...
\section{Decomposição do movimento observado: regressão de dois fatores com encolhimento}
...
neste trabalho são fixados, e cada um tem justificação própria. Para o mercado adota-se
$\beta_{\text{ref}} = 1{,}0$, que corresponde à hipótese de a ação acompanhar o índice na ausência
de informação local fiável, e é o valor para o qual a média transversal dos betas tende por
construção do próprio índice. Para o fator setorial, já ortogonalizado face ao mercado, adota-se
$\beta_{\text{ref}} = 0{,}0$, como escolha de regularização para ausência de exposição setorial
adicional quando a estimativa é imprecisa. A média nula do fator ortogonalizado não implica um
coeficiente nulo. O desvio-padrão comum de $0{,}5$
determina a rapidez da transição entre a estimativa amostral e o valor de referência: fixa-o de
modo que uma janela com erro-padrão dessa mesma ordem receba peso aproximadamente igual das duas
origens, o que situa o ponto de equilíbrio dentro da gama de erros-padrão efetivamente observada em
janelas de vinte dias. A estimação destes valores a partir da amostra não foi realizada, e a razão
é declarada: doze a dezassete empresas não constituem um corte transversal representativo, pelo que
um prior estimado sobre elas teria aparência empírica sem o ser.
...
\begin{tikzpicture}[font=\small, >={Stealth[length=2.2mm]}, x=1cm, y=1cm]
  ...
  \node[font=\footnotesize, black!60, align=left, text width=2.1cm]
    at (9.6,3.4) {as três parcelas somam sempre o movimento do dia};
\end{tikzpicture}
\caption[Um movimento repartido em três parcelas]{Repartição de um movimento da AMD em retornos
logarítmicos. A data das barras não ficou preservada no registo desta avaliação. As componentes de mercado e de setor atuaram em sentido
...
Neste caso da AMD, o retorno logarítmico de $+6{,}2944\%$,
...
\section{Recuperação de precedentes: embeddings de frase e semelhança do cosseno}
...
\begin{tikzpicture}[font=\small]
  ...
  \node[black!50, font=\footnotesize\itshape] at (1.1,4.35) {resultados};
...
frases com vocabulário comum (``record'') e assunto distinto ficam afastadas. A representação é
...
conservadora que evita atribuir à notícia a parte do movimento anterior à sua publicação.
 A Figura~\ref{fig:met_evento} representa as duas decisões.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\footnotesize,
  ses/.style={draw=black!45, fill=black!3, minimum width=1.35cm, minimum height=0.62cm},
  fech/.style={draw=black!80, fill=black!12, line width=0.9pt, minimum width=1.35cm,
           minimum height=0.62cm},
  leg/.style={font=\scriptsize, text=black!65, align=center},
  seta/.style={-{Stealth[length=4pt]}, draw=black!55},
]
  \node[fech] (s0) at (0.0,0) {\scriptsize \textbf{0}};
  \node[ses] (s1) at (1.45,0) {};
  \node[ses] (s2) at (2.9,0) {};
  \node[ses] (s3) at (4.35,0) {};
  \node[ses] (s4) at (5.8,0) {};
  \node[ses] (s5) at (7.25,0) {};
  \node[leg] at (1.45,-0.95) {+1};
  \draw[seta] (s0.south) ++(0,-0.02) .. controls +(0.55,-0.6) and +(-0.55,-0.6) .. (s1.south);
  \node[leg] at (4.35,-0.95) {+3};
  \draw[seta] (s0.south) ++(0,-0.02) .. controls +(0.55,-0.6) and +(-0.55,-0.6) .. (s3.south);
  \node[leg] at (7.25,-0.95) {+5};
  \draw[seta] (s0.south) ++(0,-0.02) .. controls +(0.55,-0.6) and +(-0.55,-0.6) .. (s5.south);
  \node[leg] at (3.62,-1.42) {dias de negociação};
  \node[leg, anchor=east] (sab) at (-1.25,0.8) {notícia de sábado};
  \draw[seta, densely dashed] (sab.east) -- ++(0.42,0) -- (s0.north);
  \node[leg, anchor=west] at (0.5,0.8) {alinhada ao primeiro\\ dia de negociação};
  \node[leg, anchor=north, text width=3.6cm] at (0.1,-1.92) {a medição começa no fecho\\ deste dia, não na abertura};
  \node[leg, anchor=north, text width=4.6cm] at (5.3,-1.92) {o movimento anterior à publicação fica de fora};
\end{tikzpicture}
\caption[A janela do estudo de evento e as duas decisões que a fixam]{As duas decisões de detalhe que condicionam todos os desfechos medidos neste trabalho. Uma notícia publicada fora de um dia de negociação é alinhada com o primeiro dia seguinte em que o mercado abre, e a medição inicia-se no fecho desse dia, assinalado a cheio. A segunda decisão é conservadora por construção: começar na abertura atribuiria à notícia a parte do movimento que a antecedeu.}
\label{fig:met_evento}
\end{figure}
...
O Excerto~\ref{lst:cosseno} reproduz a comparação tal como é executada sobre a base de casos inteira. A linha que decide o comportamento do sistema é a última: um registo sem vetor utilizável recebe semelhança zero e cai para o fim da ordenação, em vez de interromper o ciclo.

\begin{lstlisting}[caption={[A comparação no núcleo da recuperação]A comparação no núcleo da recuperação, vetorizada sobre a base de casos inteira. A decisão que importa é a última: um vetor nulo recebe semelhança zero em vez de levantar erro, porque um registo sem representação tem de cair para o fim da ordenação e não interromper o ciclo.}, label={lst:cosseno}, float=htbp]
...
\subsection{Ajuste do codificador por materialidade comparável}
\label{sec:met_qi4}

A recuperação descrita acima organiza a base de casos por \emph{tema}. Um precedente do mesmo
tema pode, no entanto, ter-se movido por uma magnitude muito diferente daquela que está em causa,
e o alerta apresenta-o ao lado dela como se fossem comparáveis. A quarta questão de investigação
pergunta se o codificador pode ser ajustado de forma que a proximidade passe a refletir também
materialidade comparável, e a Secção~\ref{sec:intro_objetivos} justifica por que razão essa
pergunta não contraria a restrição de não previsão.

\subsubsection*{Os pares de treino}

O alvo de um par de títulos é $1 - |p(a) - p(b)|$, onde $p$ é o percentil do movimento
subsequente na distribuição dos movimentos do bloco de treino. Duas razões sustentam o percentil
em vez da diferença bruta. A primeira é que os retornos têm cauda pesada: meio ponto percentual
não significa o mesmo no centro da distribuição e na cauda, e um alvo construído sobre a
diferença bruta gastaria quase todo o seu domínio a distinguir extremos entre si. O percentil é
invariante a transformações monótonas e trata as duas regiões pelo mesmo padrão. A segunda é que
o alvo fica em $[0, 1]$ por construção, que é o domínio esperado pelas funções de perda
utilizadas, sem escala arbitrária a justificar.

O mapa de percentis é ajustado \emph{apenas} sobre o bloco de treino e reutilizado nos restantes.
Ajustá-lo sobre o conjunto completo constituiria fuga de informação, e uma fuga de um tipo que
nenhuma verificação de fronteiras apanharia: a distribuição do bloco de teste entraria no treino
sem que um único par cruzasse a fronteira. O bloco de embargo fica sempre de fora, uma vez que
existe precisamente para separar treino de teste.

\subsubsection*{A amostragem, e a razão pela qual não pode ser ao acaso}

A primeira construção sorteava dois índices ao acaso. A diferença de percentis entre dois pontos
uniformes é \emph{triangular}, com média $1/3$ e densidade quase nula nos extremos, pelo que o
alvo fica concentrado em torno de um valor único: medido sobre os dados, média $0{,}665$ e desvio
$0{,}237$. Com um alvo assim, o mínimo da perda obtém-se prevendo a média para todos os pares, e
prever a média para tudo é, na prática, colapsar a representação. A
Secção~\ref{sec:av_qi4} reporta que foi exatamente isso que aconteceu.

A construção adoptada sorteia primeiro a distância desejada $d$ e só depois o ponto de partida
dentro da margem que $d$ admite, $p_a \sim U(0, 1-d)$, de modo que $|p(a) - p(b)| = d$ por
construção. Sobre os dados reais o alvo passa a ter média $0{,}499$ e desvio $0{,}289$, ou seja a
distribuição uniforme pretendida.

\subsubsection*{Os dois braços, e o que a comparação entre eles estabelece}

O braço principal treina sobre a \emph{magnitude} do movimento, tomada em valor absoluto. O braço
de replicação treina sobre o retorno \emph{com sinal}, reproduzindo neste mesmo arnês o objetivo
de direção de \textcite{du2024contrastive}. A comparação entre os dois é o que torna
verificável a distinção
entre magnitude e direção: se a direção fosse tão aprendível do texto como a magnitude, os dois
braços seriam indistinguíveis.

A extensão face a esse trabalho está no que é ajustado. \textcite{du2024contrastive} treinam
uma rede de tripletos sobre texto e séries de preço, tomando como positivo o par que partilha
o rótulo de \emph{direção}, e mantêm o codificador de frases \emph{congelado}. Aqui o
codificador é ajustado, e o objetivo do braço principal é a magnitude e não a direção.

\subsubsection*{Configuração e reprodutibilidade}

Os dois braços partem do mesmo \texttt{all-MiniLM-L6-v2} utilizado na
Secção~\ref{sec:met_recuperacao}, para que a comparação com a recuperação sem ajuste seja de igual
para igual. A perda é a \emph{CoSENT}, que ordena pares por similaridade graduada sem impor uma
escala à saída do cosseno. O treino corre sobre quarenta mil pares, uma época, lotes de trinta e
dois exemplos e taxa de aprendizagem $5 \times 10^{-6}$, com semente fixada em quarenta e dois
para o gerador do \texttt{random}, do \texttt{numpy} e do \texttt{torch}. A configuração completa
é guardada ao lado dos pesos.

\subsubsection*{As duas medidas, e o diagnóstico que as precede}

A medida do objetivo é a \textbf{comparabilidade de materialidade}: a diferença média absoluta,
em pontos percentuais, entre a magnitude do movimento da consulta e a dos precedentes devolvidos.
Valores menores são melhores. A segunda medida é a \textbf{precisão@5 por setor} da
Secção~\ref{sec:av_qi2}, e está presente para mostrar o que o ajuste custa, ou não custa,
em relevância temática. Medir apenas por setor seria avaliar o braço principal pela métrica do
braço que ele não é.

Antes de qualquer das duas ser interpretada corre um diagnóstico de degeneração, que mede o
cosseno médio entre títulos \emph{diferentes}, a norma do vetor médio e o desvio por dimensão, e
ainda quanto o espaço se deslocou face ao modelo de partida e se a geometria das semelhanças foi
preservada. A razão é que um codificador colapsado produz uma tabela de métricas plausível e sem
significado nenhum: todas as consultas devolvem os mesmos vizinhos, e nada na tabela o denuncia.


\section{Triagem de notícias: um modelo aprendido de exemplos rotulados}
...
\begin{tikzpicture}[font=\footnotesize, >={Stealth[length=2mm]},
  ...
  \node[rot] at (-2.55,0.62) {nível EMPRESA};
  \node[emp] (a) at (0,0)    {volatilidade\\ de 20 dias};
  \node[emp] (b) at (2.6,0)  {momento de\\ 5 dias};
  ...
  \node[font=\scriptsize, text=black!55, align=left, text width=2.4cm]
    at (7.9,-0.95) {variam devagar, ou nada};

  % nivel DIA
  \node[rot] at (-2.55,-1.95) {nível DIA};
  \node[dia] (h) at (0,-2.15) {retorno do\\ próprio dia};
  \node[font=\scriptsize, text=black!55, align=left, text width=5.6cm]
    at (4.3,-2.15) {igual para todos os títulos da mesma empresa nesse dia};

  % nivel NOTICIA
  \node[rot] at (-2.55,-3.15) {nível TÍTULO};
  \node[not] (i) at (0,-3.35) {comprimento\\ do título};
  \node[font=\scriptsize, text=black!70, align=left, text width=5.6cm]
    at (4.3,-3.35) {\textbf{a única que separa dois títulos da mesma empresa no mesmo dia}};
...
desse dia; \textbf{apenas uma} varia entre títulos consecutivos. A ablação da
Secção~\ref{sec:av_ablacao} mede a dependência dominante da empresa; o rótulo partilhado não permite
avaliar a distinção entre títulos do mesmo dia.}
...
A disciplina cronológica é uma afirmação sobre o que o conjunto de dados contém, e uma
afirmação dessa natureza não se estabelece por declaração. É por isso imposta por um
procedimento executável, reproduzido no Excerto~\ref{lst:lookahead}, que constrói duas séries
idênticas até ao dia da notícia e divergentes depois dele, e exige as duas propriedades que
distinguem uma entrada legítima de uma fuga: as entradas do modelo têm de permanecer iguais, e
o rótulo tem de mudar. Uma entrada que passasse a depender do futuro faria falhar a primeira
propriedade; um rótulo que não dependesse dele faria falhar a segunda.

\begin{lstlisting}[caption={[O teste que muta o futuro]O procedimento que impõe a separação
temporal. As duas séries coincidem até ao dia do acontecimento e divergem a seguir. As entradas
têm de sair idênticas, porque nenhuma delas pode observar as sessões seguintes; os rótulos têm de sair
diferentes, porque é precisamente o futuro que eles medem. O teste corre a cada execução da
verificação automática.}, label={lst:lookahead}, float=htbp]
...
Os dois parâmetros não são escolhidos à mão. O Excerto~\ref{lst:platt} reproduz o ajuste tal como é executado: uma regressão logística de uma variável sobre o bloco de validação reservado, o que é o que torna os dois valores acima reproduzíveis em vez de afirmados.

\begin{lstlisting}[caption={[Onde uma pontuação passa a ser uma probabilidade]Onde uma pontuação passa a ser uma probabilidade. Os dois parâmetros não são escolhidos: são ajustados por uma regressão logística sobre o bloco de validação reservado, o que é o que torna o valor da Equação~\ref{eq:platt} reproduzível. A classe e a função de ajuste são reproduzidas do ficheiro de origem.}, label={lst:platt}, float=htbp]
...
O treino da triagem, a calibração e a reamostragem por empresa e dia usam a semente $42$.
As cinco repetições da recuperação, incluindo a avaliação com anterioridade, usam as sementes
$42$ a $46$. A linha de base aleatória da análise do orçamento usa $40$ sementes, de $0$ a $39$.
Estas escolhas fixam as operações aleatórias de cada protocolo; não tornam iguais conjuntos
de dados recolhidos em momentos diferentes.
...
com data e desfecho. A média que acompanha os casos no alerta é descritiva e não permite antecipar
o desfecho da notícia atual. Que
esta opção produza confiança apropriada constitui uma hipótese e não um resultado, uma vez que o
estudo controlado necessário para a verificar não foi realizado.


\subsection{Utilização de ferramentas de inteligência artificial}
\label{sec:met_ia}

Esta subsecção declara, conforme a Declaração de Integridade remete, como foram utilizadas
ferramentas de inteligência artificial generativa neste trabalho e com que finalidade.

A conceção é do autor: o problema, as questões de investigação, as restrições que definem o
sistema (apenas fontes gratuitas, explicabilidade primeiro, e a recusa de prever preços), o
desenho de cada experiência, o critério de sucesso fixado antes de a correr, e todas as decisões
sobre o que construir, manter ou descartar. Nenhuma conclusão deste documento foi delegada.

A execução foi assistida, e em extensão substancial. Essas ferramentas escreveram e reestruturaram
parte considerável do código do sistema e dos seus testes, implementaram os procedimentos de
avaliação, redigiram e editaram prosa neste documento, e conduziram revisões que encontraram
defeitos no software e no texto. Onde uma medição contrariou uma afirmação do autor, a afirmação
foi retirada em vez de defendida, e o Apêndice~\ref{ap:reprodutibilidade} regista as que foram
retiradas.

O conteúdo deste documento foi revisto pelo autor. Ele, os erros que nele restem, e a sua defesa
