...
registo de decisões de produção, $98{,}0\%$ das avaliações terminam nele, contra $0{,}1\%$
no limiar de semelhança e $1{,}1\%$ na verificação de repetição no mesmo dia. A leitura não
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \foreach \nome/\valor/\tipo/\quota/\y in {
      {Não havia notícia}/{---}/{e}/{0{,}3\%}/{0.05},
      {Nomeia a empresa}/{regra}/{e}/{antes do registo}/{-0.55},
      {É recente}/{2 dias}/{e}/{antes do registo}/{-1.15},
      {Evidência suficiente}/{0{,}45}/{e}/{0{,}1\%}/{-1.75},
      {Triagem acima do piso}/{50\%}/{d}/{deixou de vetar}/{-2.35},
      {Mesma história}/{---}/{e}/{0{,}4\%}/{-2.95},
      {Já enviada hoje}/{---}/{e}/{1{,}1\%}/{-3.55},
      {Piso escalonado}/{0{,}64}/{d}/{deixou de vetar}/{-4.15},
      {Orçamento diário}/{5 por dia}/{e}/{98{,}0\%}/{-4.75}} {
    \node[lin] at (0,\y) {\nome};
  ...
  \node[lin, font=\scriptsize, text=black!65, text width=12.2cm] at (0,-5.95)
    {O ponto que elimina quase tudo é um dos escolhidos. Os dois cujo valor foi derivado de uma
     análise de custos não eliminam nada na configuração em operação, por terem deixado de vetar
     em 15 de agosto de 2026. E os dois primeiros pontos atuam antes de existir registo de
     etapa, pelo que a fração que eliminam não é observável aqui.};
\end{tikzpicture}
\caption[As sete regras, a origem do seu valor e o que cada uma elimina]{Cada ponto de decisão,
a origem do valor que utiliza e a fração das avaliações que nele termina, medida sobre o registo
de decisões de produção. A distinção entre valor derivado e valor escolhido é a mesma da
...
orçamento, cujo valor foi escolhido. As frações somam menos de cem por cento porque
$0{,}1\%$ das avaliações atravessam todas as portas e dão origem a um alerta, e porque os dois
primeiros pontos de decisão atuam antes de existir registo de etapa.}
...
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]},
  ...
  \node[cx]  (s) at (7.5,0)  {teste\\ \emph{single use}};
  ...
  \node[art] (p) at (0,-1.9)   {the \textbf{mesmo} model, in production};
