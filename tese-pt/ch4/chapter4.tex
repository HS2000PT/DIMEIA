...
reproduzido do registo do canal, com a supressão dos marcadores tipográficos e a numeração das
...
ajustado sobre o primeiro. Omite-se a documentação interna da função.}, label={lst:contrib}]
...
A empresa da Figura~\ref{fig:sis_empresa} constitui um caso ilustrativo da segunda questão de
...
provinha do mercado, com $-0{,}61\%$, e do setor, com $-0{,}22\%$. A variação percentual isolada
indica que a empresa caiu e conduz à procura de uma causa interna que não existe; a repartição
mostra que a empresa subiu contra um mercado em queda. O veredicto classifica o dia como comum:
...
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]},
  ...
  \node[cx]  (s) at (7.5,0)  {teste\\ \emph{uso único}};
...
exposta. Nela, cada frase tem de citar um facto do conjunto de evidência que a acompanha, e uma
verificação recusa a frase cujo número não pertença ao facto que ela cita. Sobre um corpo de
vinte e três tentativas de contornar essa verificação, todas foram recusadas, e sobre oito textos
fiéis nenhum o foi; nas secções efetivamente compostas não se registou uma única violação
entregue. A razão para a não expor não é o desempenho dessa verificação, mas a natureza da
garantia: no texto entregue ao utilizador a composição parte de um vocabulário fechado, ao passo
que a verificação da camada gerada enumera o que proíbe, e uma enumeração do proibido é mais
fraca do que uma enumeração do permitido. Nenhuma comparação com uma arquitetura completa de
geração aumentada por recuperação foi realizada, pelo que nada se afirma sobre o que ela
acrescentaria.
