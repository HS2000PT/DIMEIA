...
utilizador, a construção da mensagem entregue, e o ciclo de vida do componente aprendido. Quem o leia fica a saber o percurso de uma notícia desde a recolha até à mensagem, e por que razão nove em cada dez candidatas não chegam a ser comunicadas.
...
\begin{tikzpicture}[
  ...
  \node[fonte] (p) {\textbf{Preços}\\ \footnotesize cadeia de cinco\\ \footnotesize fontes};
  ...
  \node[proc, right=0.9cm of rec, text width=2.8cm] (exp)
    {\textbf{Explicação}\\ \footnotesize composta a\\ \footnotesize partir dos objetos\\ \footnotesize calculados};
...
de mensagens, por pedido \gls{HTTP}. A aplicação web que expõe o registo de decisões corre como processo
separado e lê o mesmo armazenamento de dados, sem escrever nele. Esta segunda separação garante que
...
Os preços são obtidos através de uma cadeia de cinco fontes, consultadas por ordem fixa,
...
A ordem da cadeia não é arbitrária nem definitiva. Uma das fontes foi despromovida ao passar a
responder com uma verificação anti-robô, que a torna inutilizável de forma fiável em operação
automática; permanece na cadeia, em posição inferior, como tentativa oportunista. A decisão
resultou da observação do que a fonte devolvia, e não de preferência declarada.
...
observado. A segunda é a base viva, alimentada pela varredura, com $11\,445$ casos à data
de 20 de agosto de 2026, e que cresce com a operação. A recuperação
...
Figura~\ref{fig:sis_caminho}. Cinco dessas etapas são pontos de decisão em que a candidata pode
...
\begin{tikzpicture}[
  ...
  \node[porta, left=4mm of f] (g) {já enviada\\ hoje?};
...
incide, e são apresentados na mesma. A opção é deliberada e decorre da formulação da segunda
...
$+5{,}05\%$. O sistema apresenta os três individualmente, acompanhados da amplitude e da média.
A média resume apenas os casos exibidos; isolada, ocultaria a inexistência de um desfecho comum
a casos semelhantes e não sustentaria uma previsão para a notícia atual.
...
O efeito conjunto dos pontos de decisão é considerável. Sobre as notícias datadas entre 1 e 3 de
setembro de 2026, o sistema avaliou $743$ títulos distintos de doze empresas e entregou $15$
alertas, o que corresponde a uma razão aproximada de um para cinquenta. A janela é curta, e o
registo de decisões que a sustenta conserva três dias, o que fixa o seu limite. A
Figura~\ref{fig:sis_seletividade} apresenta a distribuição por empresa.

O orçamento é, na configuração em operação, a restrição que efetivamente atua. Sobre o
registo de decisões de produção, e em cada um dos seis dias medidos, entre $77{,}7\%$ e
$99{,}8\%$ das avaliações terminam nele, contra um máximo de $4{,}6\%$ no limiar de semelhança
e de $14{,}0\%$ na verificação de repetição no mesmo dia. A leitura não
é a de que as restantes portas sejam inúteis: é a de que a redução de volume que importa
já ocorreu antes delas, no filtro de relevância e na seleção de um título por empresa por
ciclo, e que a partir daí a quantidade governada é o total diário. É também a consequência
direta da alteração de política descrita na Secção~\ref{sec:av_producao}: o modelo deixou
de exercer veto e passou a ordenar dentro do orçamento, pelo que o orçamento ser a
restrição ativa é a política a comportar-se como foi desenhada, e não um desvio dela.

Dois esclarecimentos condicionam a leitura destes números, e o segundo é o mais importante. O
primeiro é o de que a unidade é o título distinto: o sistema reavalia os mesmos títulos a cada
ciclo, pelo que o registo contém $1\,321$ linhas para os $743$ títulos, e contar avaliações
inflacionaria o funil. O segundo é o de que o total de quinze alertas não mede a matéria-prima
disponível, mas a política: o orçamento diário de cinco foi integralmente utilizado nos três dias,
pelo que quinze é o teto que a política impõe e não uma quantidade observada. O que a repartição
por empresa mede é a ordenação dentro desse teto, e essa não é imposta: nenhuma empresa dispõe de
quota própria.
...
\begin{axis}[
  ...
  xlabel={Títulos distintos na janela observada},
  ...
  \addplot[only marks, mark=square*, mark size=2.4pt, black!75,
           nodes near coords, point meta=explicit symbolic,
           every node near coord/.append style={font=\tiny, black!75, anchor=east, xshift=-4pt}]
...
\end{axis}
\end{tikzpicture}
\caption[Seletividade do sistema por empresa]{Títulos distintos avaliados e alertas
efetivamente entregues, por empresa, entre 1 e 3 de setembro de 2026. O total é de $743$ títulos e
$15$ alertas, e as duas quantidades são contadas sobre a mesma janela. O total de alertas é o teto
que o orçamento diário impõe, e não uma medição; o que a figura mostra é a repartição desse teto,
que a política não fixa. Oito das doze empresas recebem pelo menos um alerta, e a empresa com mais
títulos avaliados não é a mais alertada. A Secção~\ref{sec:av_producao} examina o efeito desta
política sobre uma janela muito maior.}
...
\begin{tikzpicture}[font=\footnotesize]
  ...
  \node[left, font=\scriptsize, black!70, align=right] at (-4.85,-0.31) {à entrada do funil};
  \fill[black!14, draw=black!45] (-1.92,-0.82) rectangle (1.92,-1.44);
  \node[font=\scriptsize, black!75] at (0,-1.13) {2066};
  \node[left, font=\scriptsize, black!70, align=right] at (-4.85,-1.13) {sem precedente forte};
  ...
  \node[left, font=\scriptsize, black!70, align=right] at (-4.85,-1.95) {abaixo do piso da triagem};
  ...
  \node[left, font=\scriptsize, black!70, align=right] at (-4.85,-2.77) {piso escalonado};
  ...
  \node[left, font=\scriptsize, black!70, align=right] at (-4.85,-3.59) {história já enviada};
  ...
  \node[left, font=\scriptsize, black!70, align=right] at (-4.85,-4.41) {orçamento diário esgotado};
  \node[right, font=\scriptsize, black!55] at (4.85,-4.41) {$-$21};
\end{tikzpicture}
\par\smallskip
\fbox{\parbox{0.78\textwidth}{\centering\small
  \textbf{5 mensagens entregues}\quad$\ne$\quad\textbf{333 passagens pelas portas}\\
  Uma avaliação não é um título distinto; os mesmos títulos foram reavaliados.}}
\caption[Distribuição completa das avaliações por ponto de decisão]{As $5\,060$ avaliações
registadas em parte de 15 de agosto de 2026. Cada banda representa o que resta depois da porta
indicada, e o valor à direita o que essa porta eliminou; as eliminações somam $4\,727$ e deixam
$333$ das $5\,060$ avaliações. As cinco mensagens entregues são uma contagem separada,
não uma categoria de avaliações. O orçamento diário foi integralmente utilizado. A data é
também a do dia em que o piso de triagem deixou de exercer veto e passou a ordenar, pelo que a banda
de $1\,194$ avaliações eliminadas nesse ponto pertence à configuração anterior à descrita na
Tabela~\ref{tab:sis_portas}; nos dias seguintes esse ponto deixa de eliminar candidatas. O
mesmo se aplica à banda do piso escalonado, que a Figura~\ref{fig:sis_razoes} regista a
zero nos seis dias posteriores.}
\label{fig:sis_funil}
\end{figure}
...
como as restantes, e existe um teste automático que falha caso volte a ser omitida. Os valores da Figura~\ref{fig:sis_funil} são anteriores a essa correção.

\subsection{Limiares e a sua proveniência}
...
\begin{table}[!htbp]
\caption[Limiares da política de decisão e a sua proveniência]{Constante associada a cada ponto de
decisão e origem do respetivo valor. O ponto que verifica se o título nomeia a empresa não
utiliza constante, por corresponder a uma regra de correspondência textual. O orçamento diário atua depois dos pontos de decisão
...
Evidência suficiente & $0{,}45$ & Escolhido. Eliminava mais candidatas na configuração anterior;
na atual predomina o orçamento diário. \\
...
O limiar de semelhança requer tratamento separado, por ser o menos fundamentado dos sete. Numa
varredura medida em julho de 2026, quando a lista vigiada tinha dez empresas e o orçamento diário
ainda não existia, sete candidatas foram eliminadas neste ponto, quatro delas por diferenças de
$0{,}04$ ou inferiores, e era então o ponto que mais eliminava. Deixou de o ser: sob a política
em operação, a ordem de grandeza do que cada ponto elimina é a da
Figura~\ref{fig:sis_razoes}. Foi testada a
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  % Cabecalhos
  \node[lin, font=\scriptsize\bfseries, text=black!60] at (0,0.75) {PONTO DE DECISÃO};
  \node[val, font=\scriptsize\bfseries, text=black!60] at (4.9,0.75) {VALOR};
  \node[lin, font=\scriptsize\bfseries, text=black!60] at (7.0,0.75) {ORIGEM};
  \node[cor, font=\scriptsize\bfseries, text=black!60] at (12.4,0.75) {DO QUE ELIMINA};
  \draw[black!25] (0,0.5) -- (12.4,0.5);

  % (nome, valor, derivado?, quota, y)
  \foreach \nome/\valor/\tipo/\quota/\y in {
      {Nomeia a empresa}/{regra}/{e}/{antes do registo}/{0.05},
      {Não havia notícia}/{---}/{e}/{0 a 3{,}9\%}/{-0.55},
      {É recente}/{2 dias}/{e}/{0 a 3{,}5\%}/{-1.15},
      {Evidência suficiente}/{0{,}45}/{e}/{0 a 4{,}6\%}/{-1.75},
      {Triagem acima do piso}/{50\%}/{d}/{deixou de vetar}/{-2.35},
      {Mesma história}/{---}/{e}/{0 a 1{,}8\%}/{-2.95},
      {Já enviada hoje}/{---}/{e}/{0 a 14{,}0\%}/{-3.55},
      {Piso escalonado}/{0{,}64}/{d}/{0\% nos seis dias}/{-4.15},
      {Orçamento diário}/{5 por dia}/{e}/{77{,}7 a 99{,}8\%}/{-4.75}} {
    \node[lin] at (0,\y) {\nome};
    \node[val] at (4.9,\y) {\valor};
    \ifthenelse{\equal{\tipo}{d}}
      {\node[der, minimum width=2.6mm, minimum height=2.6mm, inner sep=0] at (7.15,\y) {};
       \node[lin, font=\scriptsize, text=black!70] at (7.4,\y) {derivado};}
      {\node[esc, minimum width=2.6mm, minimum height=2.6mm, inner sep=0] at (7.15,\y) {};
       \node[lin, font=\scriptsize, text=black!70] at (7.4,\y) {escolhido};}
    \node[cor] at (12.4,\y) {\quota};
  }

  \draw[black!25] (0,-5.35) -- (12.4,-5.35);
  \node[lin, anchor=north west, font=\scriptsize, text=black!65, text width=12.2cm] at (0,-5.65)
    {O ponto que elimina quase tudo é um dos escolhidos, e é-o em todos os dias medidos. Dos dois
     cujo valor foi derivado de uma análise de custos, o piso da triagem deixou de vetar em 15 de
     agosto de 2026 e o piso escalonado continua armado e nada eliminou em nenhum dos seis dias
     medidos, por o orçamento se esgotar
     antes de uma empresa alcançar um segundo alerta. O filtro de relevância atua antes de existir
     registo de etapa, pelo que a fração que elimina não é observável aqui.};
\end{tikzpicture}
\caption[Os nove pontos de decisão, a origem do seu valor e o que cada um elimina]{Cada ponto de decisão,
a origem do valor que utiliza e a fração das avaliações que nele termina, medida sobre o registo
de decisões de produção em seis dias, entre 19 e 21 de agosto e entre 3 e 5 de setembro de 2026.
A coluna apresenta a amplitude entre esses dias e não um valor único, uma vez que o registo
conserva apenas três dias de cada vez e as frações variam com o dia. A unidade é a avaliação
e não a notícia: o sistema repontua os mesmos títulos a cada ciclo de sessenta segundos, pelo que
estes valores medem onde o tempo do sistema é gasto e não quantas histórias distintas cada ponto
eliminou. A distinção entre valor derivado e valor escolhido é a mesma da
Tabela~\ref{tab:sis_portas}. Os nove incluem os cinco representados na
Figura~\ref{fig:sis_caminho} e mais quatro que atuam fora dela: a ausência de notícia, a
supressão de histórias repetidas, o piso escalonado e o orçamento diário. A leitura que a
figura permite, e que a tabela sozinha não dá, é a de que a redução de volume não é feita
pelos pontos cujo valor foi derivado: é feita pelo
orçamento, cujo valor foi escolhido, e assim é em cada um dos seis dias. Os valores provêm do registo de decisões publicado com o sistema.}
...
casos recuperados em vez da totalidade, e a razão não é de espaço disponível. A investigação sobre
...
do canal, com a supressão dos marcadores tipográficos e a numeração das
quatro partes acrescentada; nenhum número, palavra ou sinal de pontuação foi alterado. Os números
entre parênteses retos identificam as quatro partes da estrutura. Todos os
...
A última linha do alerta não se limita a apresentar a probabilidade: nomeia as entradas que a
elevam e as que a reduzem. Essa enumeração não é redigida, é lida do próprio cálculo. O
Excerto~\ref{lst:contrib} mostra a operação que a produz. Numa regressão logística com
normalização, o logit é a soma do termo constante com o produto de cada coeficiente pelo valor
normalizado da respetiva entrada, pelo que cada parcela dessa soma é a contribuição exata de uma
entrada, e não uma aproximação obtida sobre o modelo depois de treinado. As centenas de
dimensões do vetor do título são somadas numa só parcela, para que a mensagem permaneça legível;
as entradas de contexto aparecem uma a uma. É desta ordenação que saem as palavras
\emph{``raised by recent volatility (20d) and sector; lowered by today's own move''} da
Figura~\ref{fig:sis_alerta}.

\begin{lstlisting}[caption={[A repartição aditiva que a mensagem enuncia]A repartição que a
última linha do alerta enuncia. O produto do vetor de coeficientes pelo vetor normalizado dá a
contribuição de cada entrada para o logit; as parcelas são agrupadas e ordenadas por módulo
decrescente. A explicação apresentada ao utilizador é esta soma, e não um segundo modelo
ajustado sobre o primeiro. Omite-se a documentação interna da função.}, label={lst:contrib}, float=htbp]
...
Secção~\ref{sec:sis_politica}. O desenho da interface não constitui contribuição desta
dissertação, pelo que não é descrito; a sua apresentação justifica-se por ser o único local em que
a ausência de comunicação por parte do sistema é observável. As Figuras~\ref{fig:sis_pagina},
\ref{fig:sis_empresa}, \ref{fig:sis_conta} e~\ref{fig:sis_silencio} apresentam, por essa ordem, o estado do
dia, a evidência de uma empresa, a aritmética que sustenta essa evidência, e o registo das
decisões tomadas sobre as notícias de uma empresa.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/app_v9_painel.png}
\caption[Estado do dia na aplicação web]{Aplicação web a 9 de setembro de 2026, com a sessão
norte-americana fechada. O cabeçalho conta os movimentos que o detetor assinalou como invulgares,
e a lista ao lado contém as doze empresas vigiadas ordenadas por esse mesmo critério, cada uma
com o seu estado à superfície. Escolher uma empresa governa a página inteira: a análise, o gráfico
e a lista de acontecimentos por baixo seguem-na todos, e alargar a lista de novo a todas as
empresas é um controlo separado, uma vez que essa escolha não muda a empresa em análise. As capturas são geradas a partir de um
instantâneo da própria aplicação, e não recolhidas à mão, com a largura de captura fixada no
gerador de cada uma, por ser dela que depende o texto da interface permanecer legível depois de
reduzido à caixa de texto deste documento.}
\label{fig:sis_pagina}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.94\textwidth]{figures/app_v9_empresa.png}
\caption[Evidência de uma empresa]{Vista de uma empresa: o veredicto em palavras antes de qualquer
número, a repartição do movimento nas três parcelas, e a série de preço. As marcas separam as duas
coisas que este trabalho distingue: o ponto assinala um dia em que um alerta foi entregue, e a seta
um dia que o detetor assinalou e que nenhuma mensagem acompanhou. A faixa inferior mostra o valor
de $z$ desses dias contra o limiar de $\pm1{,}5$, em eixo próprio por não partilhar unidade com o
preço. A direção da seta é a do fecho e nada mais; a data e o valor de $z$ de cada dia estão nas
fichas por baixo, e não sobre a linha, para não se sobreporem. A interface abre no dia corrente; a
figura mostra o intervalo de seis meses, por ser aquele em que a distinção entre assinalar e
comunicar se torna observável. A linha por baixo da repartição declara a qualidade
do ajuste que a produziu, comparada com a mediana das dezassete empresas do mapa de setores, e é o que permite ao leitor
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{figures/app_v9_conta.png}
\caption[A aritmética da repartição, tal como a interface a apresenta]{O painel que reconstrói
as três parcelas da ilustração anterior. É o único sítio do produto onde o leitor pode refazer um
número em vez de o aceitar, e é essa a razão de o mostrar: a equação, a equação com os valores do
dia substituídos, e o coeficiente de determinação enunciado nos termos da pergunta a que responde.
O terceiro passo é o que mais importa, e é a resposta ao que um leitor pergunta ao ver três
parcelas que somam exatamente: o mercado e o setor são produtos, uma sensibilidade multiplicada
pelo que esse fator fez no dia, ao passo que a parcela da empresa não é calculada de todo. É o
resto, pelo que a soma fecha por construção e tudo o que o modelo não conseguiu captar é atribuído
à empresa. É a mesma propriedade que o artefacto da Secção~\ref{sec:met_decomposicao} declara,
enunciada onde o leitor encontra o número.}
\label{fig:sis_conta}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.72\textwidth]{figures/app_v9_decisoes.png}
\caption[Registo das decisões tomadas sobre as notícias de uma empresa]{As decisões registadas
para uma empresa num dia, cada uma nomeando o título a que diz respeito e, quando a mensagem foi
retida, o motivo concreto. O silêncio é aqui uma decisão registada, e não ausência de informação.
A interface declara o que o registo não estabelece, na linha que o fecha: são as decisões que
ficaram registadas, e a lista não implica que um título tenha atravessado todas as portas da
política da Secção~\ref{sec:sis_politica}, uma vez que um título detido na primeira porta nunca
chega às seguintes.}
\label{fig:sis_silencio}
\end{figure}

A empresa da Figura~\ref{fig:sis_empresa} constitui um caso ilustrativo da segunda pergunta do
investidor, pela discordância entre a variação percentual e a repartição. A ação caiu $0{,}97\%$, e a
contribuição da própria empresa foi \emph{positiva}, de $+0{,}11\%$: quem a puxou para baixo foram
o mercado, com $-0{,}53\%$, e o setor, com $-0{,}55\%$. Um investidor que lesse apenas a percentagem
concluiria que algo se tinha passado na empresa, e a repartição diz o contrário. As três parcelas
somam ao movimento observado por construção, como a Figura~\ref{fig:sis_conta} mostra, pelo que o
fecho da soma não é prova de que a repartição esteja bem estimada; e esta repartição estatística
não exclui a existência de causas externas não modeladas. A qualidade do ajuste reportada para o dia,
$0{,}44$, fica abaixo da mediana de $0{,}460$ das dezassete empresas do mapa de setores, e a interface
assinala a repartição como indicativa por essa razão. O veredicto classifica o dia como comum:
$128$ dos últimos $248$ dias de negociação registaram variação igual ou superior.


A mesma figura mostra a ressalva que a acompanha. O coeficiente de determinação desta empresa é
de $0{,}44$, abaixo da mediana de $0{,}460$ das dezassete empresas do mapa de setores, e a interface
declara-o por baixo da repartição, com a instrução de a ler como indicativa. As duas afirmações
convivem de propósito: a repartição é a melhor estimativa disponível e, nesta empresa, o mercado e
o setor acompanham menos de metade da variação diária. Omitir a segunda tornaria a primeira mais persuasiva do que
o ajuste que a sustenta, que é o defeito que a Secção~\ref{sec:con_limitacoes} nomeia.

A Figura~\ref{fig:sis_silencio} mostra a mesma exigência do lado do silêncio. Cada decisão
nomeia o título a que diz respeito e, quando a mensagem foi retida, o critério que a reteve, de
modo que o leitor distingue um título que o sistema nunca julgou material de uma que julgou
material e suprimiu por um motivo declarado à partida. Nenhuma das decisões é apresentada como
falha, o que evita a leitura inversa que uma versão anterior da interface permitia. O registo é
também explícito sobre o que não estabelece: enumera as decisões que ficaram registadas, e um
título detido na primeira porta nunca chega às seguintes, pelo que a lista não é uma travessia
da política.
...
\begin{tikzpicture}[font=\footnotesize, >={Stealth[length=2.2mm]},
  ...
  \node[esp] (b) at (3.55,0)  {guardado como candidato, com o seu vetor};
  \node[et]  (c) at (7.0,0)   {\textbf{dia $t+8$}\\ o impacto é observável};
  \node[et]  (d) at (10.5,0)  {entra na \textbf{base de casos}};
  ...
  \draw[->, thick, black!55] (d.south) -- ++(0,-0.85) -| (a.south)
    ...
    {as avaliações seguintes comparam com os casos já maturados};
\end{tikzpicture}
...
O mecanismo atualiza a evidência exibida, sem tocar nos parâmetros do modelo. Na taxonomia de deriva de
...
estavam a ser publicados no armazenamento de dados partilhado. Corresponde à classe de custo que
...
validação e as comparações são realizadas no bloco de teste reservado. A reutilização desse bloco
limita a independência das comparações, como discutido na Secção~\ref{sec:av_qi3}.
Na metade inferior, a observação: o modelo
...
\begin{description}
  ...
  \item[Verificação da separação temporal.] Aos blocos cronológicos e ao embargo junta-se um
        teste executável, reproduzido no Excerto~\ref{lst:lookahead}: perturbado o futuro de um
        exemplo, as suas entradas têm de permanecer inalteradas e o seu rótulo tem de mudar. A
        ausência de fuga passa assim de afirmação a propriedade verificada a cada execução.
  ...
  \item[Versionamento dos artefactos.] O modelo treinado, o calibrador e um descritor da execução,
        contendo dimensão de cada bloco, prevalência, semente e métricas, são conservados com a
        dissertação.
...
Uma camada de geração ancorada encontra-se, ainda assim, implementada e avaliada, e não é
exposta ao utilizador. A Secção~\ref{sec:sis_inteligencia} descreve-a, com a garantia que oferece
e a razão pela qual permanece fechada.
...
alteração exige tempo de observação que, à data em que este capítulo foi escrito, não existia.

A instrumentação que o tornaria possível foi construída e implantada em 4 de setembro de 2026, e
o registo passou a guardar, por decisão, o valor de cada entrada, a data da última barra de preço
utilizada e a identidade do modelo. Duas restrições limitam o que dela se poderá retirar, e ambas
foram fixadas antes de existir qualquer candidato. A primeira é que apenas são utilizáveis as
decisões cuja barra de preço seja anterior ou igual à data da notícia, uma vez que uma barra
posterior descreveria um mercado que já observou o desfecho que o rótulo mede. A segunda é que a
unidade de análise é o par constituído pela empresa e pelo dia, e não o título, o que reduz
substancialmente o número de observações independentes disponíveis numa janela curta. Nada se
afirma neste documento sobre o resultado dessa comparação.
...
\gls{PSI} de qualquer entrada ultrapassa $0{,}25$, o limiar convencional
...
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]},
  ...
  \node[cx]  (t) at (8.7,-1.9) {retreino sobre janela de 24 meses};
  ...
  \draw[->, dashed, black!45] (q.south) -- ++(0,-0.55)
    node[below, align=center, black!60] {não supera:\\ regista-se e descarta-se};
\end{tikzpicture}
...
\section{A camada de inteligência}
\label{sec:sis_inteligencia}

As secções anteriores descreveram um sistema que compõe texto a partir de objetos calculados, com
um vocabulário fechado. Esta secção descreve a camada que faz o mesmo com um modelo de linguagem,
sob uma garantia diferente, e a razão pela qual não está exposta ao utilizador.

A arquitetura corrente para responder a uma pergunta sobre um corpo de documentos é a geração
aumentada por recuperação \autocite{lewis2020rag}, discutida na Secção~\ref{sec:ctx_rag}. O
sistema em produção executa a primeira metade e detém-se antes da segunda. A camada aqui descrita
executa as duas, e existe para determinar o que custaria fazê-lo com a mesma disciplina de
evidência.

\subsection{O contrato: a linguagem não produz factos}

O objeto que atravessa esta camada é um \emph{pacote de evidência}. Cada facto que dele consta traz
um identificador citável e a origem declarada, que é uma de três: medido a partir de preços,
calculado por um dos motores do Capítulo~\ref{cap:metodologia}, ou produzido pelo modelo de triagem.
Nenhum facto tem origem \emph{gerada}. A separação é a regra que estrutura a camada: o modelo de
linguagem escreve a língua e nunca os números.

A Figura~\ref{fig:sis_inteligencia} representa a travessia. Duas chamadas ao modelo, e a divisão
entre elas é deliberada. A primeira não escreve texto: traduz a pergunta num plano que o sistema
sabe executar. A segunda não decide nada: redige sobre factos que já existem. Entre as duas, são
os componentes descritos neste capítulo que produzem a evidência, e nenhuma das chamadas tem
acesso ao mercado.

Um encaminhador determinístico cobre a primeira chamada quando não existe chave de interface
disponível. Não é um recuo de conveniência: a restrição de utilizar apenas fontes gratuitas
implica que a camada tem de continuar a responder quando o limite de utilização acabar, e uma
interface conversacional que emudece nessa altura não serviria o público que o
Capítulo~\ref{cap:introducao} define.

\subsection{Dois níveis de garantia, e o mais fraco é este}

A verificação de ancoragem recusa a frase cujo número não pertença ao facto que ela cita. A
formulação importa: a ligação é por frase e não pelo pacote inteiro, porque de outro modo
qualquer número do pacote poderia ser colado a qualquer afirmação.

A garantia não é, contudo, da mesma natureza da que o alerta oferece, e a
Tabela~\ref{tab:sis_garantias} separa as duas.

\begin{table}[!htbp]
\caption[Os dois níveis de garantia sobre o texto entregue]{As duas formas de garantir que o texto
não afirma mais do que os componentes calcularam. A diferença não é de eficácia medida, mas de
natureza: uma enumeração do que é permitido é fechada, e uma enumeração do que é proibido é uma
lista que pode não estar completa.}
\label{tab:sis_garantias}
\centering
\small
\begin{tabular}{L{0.20\textwidth} L{0.36\textwidth} L{0.34\textwidth}}
\toprule
 & \tabhead{Alerta entregue} & \tabhead{Texto desta camada} \\
\midrule
...
A assimetria justifica-se pelo risco e não pelo desempenho. O alerta interrompe o utilizador sem
que ele o tenha pedido, e o texto desta camada é consultado por quem já está a olhar para a
evidência que o acompanha.

\subsection{O que foi medido, e o que não se afirma}

A verificação foi confrontada com um conjunto de tentativas de a contornar. Sobre vinte e três
ataques, todos foram recusados; sobre oito textos fiéis, nenhum o foi; e nas secções efetivamente
compostas não se registou uma única violação entregue.

Estes números delimitam-se a si próprios. Os dois primeiros são determinísticos e reproduzem-se
sobre o mesmo corpo de ataques; o terceiro é uma contagem sobre uma execução concreta. A força
medida é um limite inferior, uma vez que o corpo de ataques foi construído pelo próprio autor e
nada garante que esgote as formas de contornar a verificação.

A camada não está exposta, e a razão não é o desempenho da verificação. É a natureza da garantia
que a Tabela~\ref{tab:sis_garantias} descreve: expor ao utilizador um texto sob a garantia mais
fraca, quando o resto do sistema opera sob a mais forte, tornaria a promessa do produto desigual
conforme o ecrã. Nenhuma comparação com uma arquitetura completa de geração aumentada por
recuperação foi realizada, pelo que nada se afirma sobre o que ela acrescentaria.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\footnotesize,
  cx/.style={draw=black!70, rounded corners=2pt, fill=black!5, align=center,
        text width=2.36cm, minimum height=1.22cm},
  leg/.style={font=\scriptsize, text=black!62, align=center},
  seta/.style={-{Stealth[length=4pt]}, draw=black!55},
]
  \node[cx] (n0) at (0.0,0) {\textbf{pergunta}\\[1pt]\scriptsize em linguagem\\corrente};
  \node[cx] (n1) at (3.02,0) {\textbf{plano}\\[1pt]\scriptsize o que o sistema\\sabe executar};
  \node[cx] (n2) at (6.04,0) {\textbf{evidência}\\[1pt]\scriptsize factos com\\origem declarada};
  \node[cx] (n3) at (9.06,0) {\textbf{resposta}\\[1pt]\scriptsize redigida sobre\\esses factos};
  \node[cx] (n4) at (12.08,0) {\textbf{âncora aberta}\\[1pt]\scriptsize o leitor abre o\\facto citado};
  \draw[seta] (n0) -- (n1);
  \node[leg] at (1.51,0.84) {modelo lê};
  \draw[seta] (n1) -- (n2);
  \node[leg] at (4.53,0.84) {motores calculam};
  \draw[seta] (n2) -- (n3);
  \node[leg] at (7.55,0.84) {modelo redige};
  \draw[seta] (n3) -- (n4);
  \node[leg] at (10.57,0.84) {verificação};
  \node[leg, text width=10.5cm] at (6.04,-1.2) {o modelo nunca toca no mercado: a primeira chamada não escreve, a segunda não decide};
\end{tikzpicture}
\caption[A travessia de uma pergunta até ao facto que a sustenta]{As duas chamadas ao modelo de linguagem estão separadas de propósito. A primeira traduz uma frase humana num plano que o sistema sabe executar e não escreve texto. A segunda redige sobre factos que já existem e não decide nada. Entre as duas, os motores descritos neste capítulo produzem a evidência, e cada facto chega com a origem declarada. A verificação final recusa a frase cujo número não pertença ao facto que ela cita, e é o que permite ao leitor abrir a âncora e confrontar a afirmação com aquilo que a sustenta.}
\label{fig:sis_inteligencia}
\end{figure}
