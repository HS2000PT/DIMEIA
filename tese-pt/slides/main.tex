...
\titlegraphic{\includegraphics[height=1.05cm]{logo_lockup.png}}
...
% ══ 1. O PROBLEMA ═══════════════════════════════════════════════════════════
% Evidencia que o deck nao tinha: os votos dos leitores do canal. Numero E ressalva no mesmo
% slide, de proposito -- e' isso que retira ao arguente a pergunta que ele ia fazer.
\begin{frame}{Serviu a alguém? A única evidência recolhida em aberto}
Cada alerta leva dois botões. \textbf{As regras foram fixadas antes de existir o primeiro voto}
e nenhuma foi alterada depois.
\vspace{2mm}
\begin{center}
\begin{tabular}{@{}lr@{}}
\toprule
Votos, um por pessoa e alerta, sobre 62 alertas & \textbf{91} \\
Pessoas & \textbf{3} \\
\midrule
Classificaram o alerta como útil & \textbf{86 de 91} $= 95\%$ \; {\scriptsize $[88;98]$} \\
O mesmo, sem o leitor que deu $58\%$ deles & \textbf{34 de 38} $= 89\%$ \; {\scriptsize $[76;96]$} \\
\bottomrule
\end{tabular}
\\[1mm]
{\scriptsize entre parênteses, o intervalo de Wilson a 95\%}
\end{center}
\vspace{1.5mm}
{\footnotesize \textbf{E o que isto não é.} Três pessoas são um piloto, não um estudo. Vota quem
quer, o que empurra a amostra para os extremos; ninguém recebeu o movimento \emph{sem}
explicação, logo nada aqui atribui a utilidade à explicação em si; e utilidade percebida não é
decisão melhor. \textbf{O estudo controlado está desenhado e não foi corrido} --- é a primeira
...
Terminal profissional & \checkmark & \checkmark & \checkmark & não publicado \\
...
\midrule
\textbf{Por significado (o método)} & $\mathbf{0{,}395}$ \\
As mais recentes & $0{,}204$ \\
Sempre o setor maior & $0{,}200$ \\
Por palavras em comum & $0{,}196$ \\
Ao acaso (o chão) & $0{,}117$ \\
\bottomrule
\end{tabular}
\vspace{2mm}
...
\begin{itemize}
  \small
  \item O corpus completo é 84\% tecnologia, e ali \textbf{devolver sempre tecnologia} vale
        $0{,}840$ sem modelo nenhum, acima de qualquer método. É por isso que a medição
        principal usa o subconjunto \textbf{equilibrado}, onde essa estratégia vale $0{,}200$
        o método a supera com folga, e a margem sobre o acaso é $+0{,}278$.
  \item O método supera o acaso nos \textbf{cinco setores}: $0{,}589$ na energia, com chão $0{,}110$.
        Não se mediu vantagem sobre um \textbf{filtro pelo setor conhecido} da consulta.
  \item Exigindo candidatos anteriores: \textbf{0{,}513}, chão \textbf{0{,}259}, margem
        \textbf{$+0{,}254$}. O teste não impõe a maturação de oito dias da produção.
  \item \textbf{Semelhança de tema não garante direção}. Os casos são apresentados um a um.
\end{itemize}
\end{frame}
...
\begin{tikzpicture}[font=\tiny, x=7.0cm, y=0.245cm]
  ...
  \node[right, text=orange!60!black] at (0.655,1.5) {passa sempre};
  \draw[black!40] (0.23,-0.5) -- (0.68,-0.5);
  % rotulo escrito a mao: `{\x}` imprimia `0.25`, com ponto decimal, num
  % deck portugues e a duas linhas do piso.
  \foreach \x/\xl in {0.25/{0{,}25}, 0.35/{0{,}35}, 0.45/{0{,}45},
                       0.55/{0{,}55}, 0.65/{0{,}65}}
    \node[below, black!55] at (\x,-0.5) {$\xl$};
\end{tikzpicture}
\end{center}
\vspace{-1mm}
\begin{center}
Em \textbf{48\%} dos títulos distintos, o resultado estava determinado\\
pela \textbf{empresa} antes de o título ser lido.
\end{center}
\vspace{1mm}
{\footnotesize Em nenhuma avaliação registada a Apple alcançou o piso, com um máximo de $0{,}472$; a AMD ultrapassou-o em todas. Contada por decisão a fração sobe a $60\%$, e é a repontuação de minuto a minuto que a inflaciona.}
\end{frame}
...
\begin{tikzpicture}[font=\scriptsize, >={Stealth[length=2mm]},
  ...
  \foreach \xn/\xlim/\xfut/\xcus in {%
    0/{Nenhuma estudo \emph{controlado} com pessoas}/{Correr o estudo: 6 a 10 pessoas}/{calendário},%
...
\includegraphics[height=0.68\textheight]{app_v9_empresa.png}
\end{column}
\begin{column}{0.35\textwidth}
\small
\textbf{1. É invulgar?}\\
{\footnotesize Em palavras, antes do número: $128$ dos últimos $248$ dias
moveram-se tanto ou mais. O veredicto é \emph{``Quiet''}, e é dito antes de qualquer valor.}
\vspace{3mm}

\textbf{2. Foi a empresa?}\\
{\footnotesize \textbf{Não, não foi.} A ação cai $0{,}97\%$ e a parcela da
própria empresa é \emph{positiva}, $+0{,}11\%$: puxaram-na para baixo o mercado
($-0{,}53\%$) e o setor ($-0{,}55\%$). E o ajuste desta empresa
fica \textbf{abaixo} da mediana.}
\vspace{3mm}
...
\end{center}
\vspace{1mm}
\begin{center}\includegraphics[width=0.97\textwidth,height=0.46\textheight,keepaspectratio]{app_v9_decisoes.png}\end{center}

{\footnotesize As decisões registadas para uma empresa, cada uma nomeando o título e,
quando a mensagem foi retida, o motivo. \textbf{É a parte que uma demonstração ao vivo não
mostra}: nove em cada dez varreduras não enviam nada, e o silêncio fica escrito como um alerta.}
