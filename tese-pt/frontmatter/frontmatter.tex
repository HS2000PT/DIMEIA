...
Três técnicas foram comparadas com linhas de base transparentes. A deteção normalizada pela
...
Three techniques were compared with transparent baselines. Volatility-normalised detection proves
...
\listoftables
% A Lista de Excertos VOLTA a ser impressa. O documento tem tres excertos, e o modelo
% oficial MEIA manda imprimir a lista quando existem. Os excertos entraram porque o juri
% le o PDF e mais nada: uma garantia afirmada e' uma afirmacao, e as tres que este
% trabalho faz -- a janela que exclui o dia avaliado, a separacao temporal imposta por um
% teste, e a explicacao que e' o proprio calculo -- so sao inspeccionaveis se o codigo que
% as produz estiver impresso aqui.
\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{Lista de Excertos}
\lstlistoflistings
...
% de glossário. A Lista de Algoritmos não consta porque o documento não tem algoritmos formais, e o
% próprio modelo manda remover esse comando nesse caso.
\cleardoublepage
\phantomsection
...
\addlinespace
$r_m$, $\tilde{r}_s$ & Retorno diário do mercado e retorno do setor ortogonalizado face ao mercado & Eq.~\ref{eq:decomposicao} \\
...
\chapter*{Como ler esta tese}
\addcontentsline{toc}{chapter}{Como ler esta tese}
\markboth{Como ler esta tese}{Como ler esta tese}

Este documento descreve um sistema que responde a três perguntas de um investidor particular, e
mede quanto vale cada uma das respostas. Tem cerca de cem páginas de corpo, e nem todos os
leitores procuram a mesma coisa. Esta página propõe quatro percursos.

\begin{description}
  \item[Quem quer saber o que o trabalho conclui] pode ler o Capítulo~\ref{cap:introducao}, que
    enuncia as três perguntas, e depois a Secção~\ref{sec:con_respostas}, onde cada uma recebe a
    sua resposta com o alcance delimitado. São cerca de doze páginas, e a
    Figura~\ref{fig:con_cadeia} resume o percurso de cada questão numa só imagem.

  \item[Quem quer saber como foi medido] deve ler o Capítulo~\ref{cap:metodologia}, que descreve
    as quatro técnicas e a disciplina temporal que impede cada uma de usar informação que ainda
    não existia, e em seguida a Secção~\ref{sec:av_desenho}, que fixa as métricas e, o que mais
    importa, o valor que cada uma assume na ausência de informação.

  \item[Quem quer saber como o sistema funciona] deve ler o Capítulo~\ref{cap:sistema}. O
    percurso de uma notícia está na Secção~\ref{sec:sis_percurso}, a política que decide o
    silêncio na Secção~\ref{sec:sis_politica}, e a camada que compõe o texto entregue na
    Secção~\ref{sec:sis_explicacao}.

  \item[Quem quer verificar] deve ir ao Apêndice~\ref{ap:reprodutibilidade}. A
    Tabela~\ref{tab:ap_numeros} liga cada resultado reportado à secção que o produz, e a
    Tabela~\ref{tab:ap_matriz} enumera cada afirmação substantiva com o estado em que ficou
    depois de medida, incluindo as que foram retiradas.
\end{description}

Três convenções atravessam o documento e vale a pena enunciá-las antes de começar. Nenhum número
foi escrito à mão: cada um é produzido por um procedimento automático e o texto cita o ficheiro
que o contém. Nenhuma comparação é apresentada sem o valor que um método sem informação obteria,
porque uma percentagem lida sozinha conduz à conclusão errada, e o documento regista três ocasiões
em que isso quase sucedeu. E cada resultado é acompanhado do que dele \emph{não} decorre, que é o
passo que distingue uma avaliação de uma demonstração.
